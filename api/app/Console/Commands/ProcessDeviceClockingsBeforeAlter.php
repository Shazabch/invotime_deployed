...
class ProcessDeviceClockingsBeforeAlter extends Command
{
        ...
        if (!$checkData) {
            ...
            if ($clocking_type == "alternate") {
                ...
                list($valid_clocking, $current_type) = $this->processAlternateClocking(
                    ...
                    $alternate_clocking_interval, $current_shift
                );
    ...
    private function processAlternateClocking($user, $dt, $current_date, $shift_id, $prev_shift_id,
                                            $overnight, $prev_overnight, $optional_overnight,
                                            $optional_prev_overnight, $prev_overnight_starts_at,
                                            $alternate_clocking_interval, $current_shift = null)
    {
        $valid_clocking = true;
        $current_type = "in";
        ...
        // Apply overnight fix for early morning clockings
        if (
            $overnight
            && !$optional_overnight
            && $current_shift
        ...
        // Fallback: Get last clocking for today
        if (!$last_clocking) {
            $last_clocking = Clockings_new::where('employee_id', $user->id)
