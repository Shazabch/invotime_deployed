...
class Default_shifts extends CI_Controller
{
	...
	function assignShifts()
	{
		...
		if (in_array('none', $public_holidays)) {
			...
			// Filter holidays by the SELECTED branch, not the logged-in user's branch.
			// get_public_holidays($branch_id) returns:
			//   - PHs with branch_id = selected branch (branch-specific)
			//   - PHs with branch_id = 0 or NULL (company-wide / all branches)
			// This ensures branch-specific PHs for OTHER outlets are not applied here.
			$public_holidays = get_public_holidays($branch_id ?: false);
		}
