...
class Employees extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['employees'] = $this->db->select('employees.*,roles.job_name,roles.permissions_level,positions.title,departments.name as department_name,sections.title as section_title,branches.name as branch_name,GROUP_CONCAT(DISTINCT eg.name ORDER BY eg.name SEPARATOR ", ") as group_names, date_format(hired_on, "%d %b, %Y") as joining_date, hired_on as joining_date_sort', false)->from('employees')->join('roles', 'employees.role_id = roles.id', 'left')->join('positions', 'employees.position_id = positions.id', 'left')->join('departments', 'employees.department_id = departments.id', 'left')->join('employee_groups_relation egr', 'egr.employee_id = employees.id', 'left')->join('employee_groups eg', 'eg.id = egr.group_id', 'left')->join('sections', 'employees.section_id = sections.id', 'left')->join('branches', 'employees.branch_id = branches.id', 'left')->where('roles.exclude_from_system', 'no')->where('employees.company_id', $cid)->where('employee_status', 'active')->where("employees.deleted_at is null $where_branch_1 $where_department")->group_by('employees.id')->order_by("special_id", "asc")->get()->result();
        ...
        $data['roles'] = $this->db->select('id,job_name')->from('roles')->where('exclude_from_system','no')->where('role_type', 'invotime')->where("company_id = $cid $where_branch_3")->order_by("job_name", "asc")->get()->result();
    ...
    public function export()
    {
        ...
        if ($this->input->get("type") === "excel") {
            ...
            // Prevent stray buffered output (e.g. BOM/whitespace) from corrupting the XLS binary stream.
    ...
    public function getSingleEmployee()
    {
        ...
        $data['roles'] = $this->db->select('id,job_name')->from('roles')->where('exclude_from_system','no')->where('role_type', 'invotime')->where('company_id', $cid)->get()->result();
    ...
    public function getPositions()
    {
        ...
        // Validate first_name: allow only letters, numbers, and spaces
        // $clean_first_name = preg_replace('/[^a-zA-Z0-9 ]/', '', $request->first_name);
        // if ($clean_first_name !== $request->first_name) {
        //     echo json_encode([
        //         'success' => false,
        //         'msg' => 'First name may only contain letters, numbers, and spaces. Please remove special characters.'
        //     ]);
        //     return;
        // }
        // $request->first_name = $clean_first_name;
    ...
    public function save()
    {
        ...
        if (count($emp_exist) > 0) {
            ...
            $emp_data = array(
                // Strip special characters from first_name, keep letters, numbers, and spaces
            ...
            if (in_array($cid, companies_allowed_for_meal_allowance())) {
                $emp_data["food_rate"] = $request->meal_rate ? $request->meal_rate : 0;
                $emp_data["nsa_rate"] = $request->nsa_rate ? $request->nsa_rate : 0;
                $emp_data["dsa_rate"] = $request->dsa_rate ? $request->dsa_rate : 0;
            }
            ...
            if ($this->db->insert('employees', $emp_data)) {
                ...
                $log_data = [
                    ...
                    'target_id' => $emp_id,
                    'target_name' => $emp_data['first_name'],
    ...
    public function update()
    {
        ...
        $current_emp = $this->db->select('employee_status')
            ->from('employees')
            ->where('id', $id)
            ->get()
            ->row();

        $current_status = $current_emp ? $current_emp->employee_status : null;

        // Only check quota if changing from inactive → active
        if ($request->employee_status == 'active' && $current_status != 'active') {
            $this->db->select('companies.package, companies.additional_staff, packages.max_outlets, packages.max_active_staff');
            $this->db->join('packages', 'packages.id = companies.package', 'left');
            $this->db->where('companies.id', $cid);
            $company_details = $this->db->get('companies')->row();
