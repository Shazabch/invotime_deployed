...
', FALSE)
			...
			// ADD THESE TWO LINES – This brings in the groups
    ...
    OR (employees.employee_status = 'resigned' AND employees.resignation_date >= DATE_FORMAT('$first_day', '%Y-%m-01'))
		...
		if (!empty($department_id)) {
			$selected_departments = $this->db
				->select('GROUP_CONCAT(DISTINCT d.name ORDER BY d.name SEPARATOR ", ") AS department_names', false)
				->from('departments d')
				->where_in('d.id', (array) $department_id)
				->get()
				->row()
				->department_names;
		}
		...
		if ($_POST['type'] == "accounts") {
		...
		} else if ($_POST['type'] == "over_time_summary") {

			// Add error reporting control at the beginning
			...
			try {
				...
				if (empty($employees)) {
					throw new Exception("No employee data found for the selected criteria.");
				}

				if (empty($first_day) || empty($last_day)) {
					throw new Exception("Date range is not properly defined.");
				}
				...
				foreach ($employees as $i => $emp) {
					try {
						...
						// Validate that calculate_summary_data returned expected structure
						if (!is_array($data)) {
							throw new Exception("Invalid data returned for employee {$emp->id}");
						}
						...
						// Log individual employee errors but continue processing others
						error_log("Error processing employee {$emp->id}: " . $e->getMessage());
						continue;
					}
				}
				...
				if (empty($all_data)) {
					throw new Exception("No valid employee data could be processed.");
				}


				// Prepare formatted dates for title and display
				$date = new DateTime($first_day);
				$data['from_f'] = $date->format('d/m/Y');
				$month_year = $date->format('F Y'); // For title - e.g., "January 2025"
				$date = new DateTime($last_day);
				$data['to_f'] = $date->format('d/m/Y');

				$data["all_data"] = $all_data;
				$data["branch_name"] = $branch_name ?? 'All Branches'; // Default if not set
				$data["department"] = $branch_name ?? 'All Branches'; // Default if not set
				...
				if (!empty($selected_departments)) {
					$title .= ' For (' . $selected_departments . ')';
				}
				...
				// Branch information
				$object->getActiveSheet()->setCellValue('A3', 'BRANCH: ' . strtoupper($data["branch_name"]));
				...
				// Add daily columns dynamically for the month
				...
				foreach ($period as $dateObj) {
					$table_columns[] = $dateObj->format('d'); // e.g., 01, 02, 03 ...
				...
				$table_columns = array_merge($table_columns, $summary_cols);
				$table_columns2 = array_merge($table_columns2, $summary_cols2);

				// Set branch info - starting at row 5 now due to additional title rows
				$column = 0;
				$info_row = 5;
				foreach ($name_columns as $field) {
					$object->getActiveSheet()->setCellValueByColumnAndRow($column, $info_row, $field);
					$object->getActiveSheet()->getStyleByColumnAndRow($column, $info_row)->getFont()->setBold(true);
					$column++;
				}

				$object->getActiveSheet()->setCellValueByColumnAndRow(0, $info_row + 1, $data["branch_name"]);
				$object->getActiveSheet()->setCellValueByColumnAndRow(1, $info_row + 1, $data["from_f"]);
				$object->getActiveSheet()->setCellValueByColumnAndRow(2, $info_row + 1, $data["to_f"]);
				$object->getActiveSheet()->setCellValueByColumnAndRow(3, $info_row + 1, date("d/m/Y H:i:s"));
				$object->getActiveSheet()->setCellValueByColumnAndRow(4, $info_row + 1, $current_user["first_name"] ?? 'System');

				// Set table headers - moved down due to additional rows
				$header_row = $info_row + 3;
				$column = 0;
				foreach ($table_columns as $field) {
					...
					// Add background color to header for better visibility
					$object->getActiveSheet()->getStyleByColumnAndRow($column, $header_row)
						->getFill()
				...
				foreach ($table_columns2 as $field) {
					...
					// Add background color to header for better visibility
					$object->getActiveSheet()->getStyleByColumnAndRow($column, $header_row2)
						->getFill()
						->setFillType(PHPExcel_Style_Fill::FILL_SOLID)
						->getStartColor()
						->setARGB('FFE6E6E6');
				...
				foreach ($all_data as $i => $r) {
					...
					try {
						$col = 0;
						$month_total_ot = "00:00";
						$month_total_ot_rd = "00:00";
						$month_total_ot_ph = "00:00";
						$shift_count = 0;
						$ot_of_days = 0;
						$ot_regular = 0;

						if (isset($r["dates"]) && is_array($r["dates"])) {
						...
						// Safe employee data access
						$employee = $r["employee"] ?? null;
						$object->getActiveSheet()->setCellValueByColumnAndRow($col++, $row, $employee->special_id ?? '');
						$object->getActiveSheet()->setCellValueByColumnAndRow($col++, $row, $employee->first_name ?? '');
						$object->getActiveSheet()->setCellValueByColumnAndRow($col++, $row, $r["employee"]->department ?? '');
						$object->getActiveSheet()->setCellValueByColumnAndRow($col++, $row, $r["employee"]->branch ?? '');
						$object->getActiveSheet()->setCellValueByColumnAndRow($col++, $row, $r["employee"]->group_names ?? '');
						// Fill daily OT for the month with safe access
						foreach ($period as $dateObj) {
							...
							$daily_ot_ph = "00:00";
							$shift_status = '';
							// Check if we have dates array (from your sample code structure)
							if (isset($r["dates"]) && is_array($r["dates"])) {
								foreach ($r["dates"] as $d) {
									if (isset($d->date) && $d->date == $dateStr) {
										$daily_ot = getOvertimeValue($d, $r['public_holidays'], $r['rest_days'], $r['off_days']);
										$month_total_ot = add_time_minus($month_total_ot, $daily_ot);
										$ot_of_days = getOvertimeValueOffDays($d, $r['public_holidays'], $r['rest_days'], $r['off_days']);
										$daily_ot_rd = getOvertimeValueRestDays($d, $r['public_holidays'], $r['rest_days'], $r['off_days']);
										$month_total_ot_rd = add_time_minus($month_total_ot_rd, $daily_ot_rd);
										$daily_ot_ph = getOvertimeValuePh($d, $r['public_holidays'], $r['rest_days'], $r['off_days']);
										$month_total_ot_ph = add_time_minus($month_total_ot_ph, $daily_ot_ph);
										$ot_regular = getOvertimeValueRegular($d, $r['public_holidays'], $r['rest_days'], $r['off_days']);
										if (!$d->is_ot) {
											// If Ot Is Rejected Then Dont Show
											$daily_ot = 0;
										}
										$decimal_ot = toDecimal($daily_ot);
										$shift_status = '';

										// Check shift
										if ($d->is_shift === true || $d->is_shift === 'true') {
											$shift_status = '';
										}

										// Check paid leave
										if (!empty($d->is_paid_leave) || $d->is_paid_leave === true || $d->is_paid_leave === 'true') {
											$shift_status = 'PL';
										}

										// Check unpaid leave
										if (!empty($d->is_unpaid_leave) || $d->is_unpaid_leave === true || $d->is_unpaid_leave === 'true') {
											$shift_status = 'UPL';
										}

										// detect real clocking (nested)
										$hasClocking = false;
										if (!empty($d->clockings) && is_array($d->clockings)) {
											foreach ($d->clockings as $c) {
												if (!empty($c->clock_in) || !empty($c->clock_out)) {
													$hasClocking = true;
													break;
												}
											}
										} elseif (($d->is_shift === true || $d->is_shift === 'true') && !$hasClocking) {
											$shift_status = 'A';
										}
										$ot_edited = false;
										///// Check OT M
										...
										// Write ONLY if > 0, otherwise write empty cell
										if ($decimal_ot > 0) {
											$decimal_ot = (float) $decimal_ot; // ensure numeric
											$object->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $decimal_ot);
											$object->getActiveSheet()->getStyleByColumnAndRow($col, $row)
												->getNumberFormat()
												->setFormatCode('0.0');

											if ($ot_edited) {           // ← adjust field name
												$object->getActiveSheet()->getStyleByColumnAndRow($col, $row)
													->getFont()
													->setColor(new PHPExcel_Style_Color('FF0000'));   // red
											}
											$col++;
										} else {
											$object->getActiveSheet()->setCellValueByColumnAndRow($col++, $row, $shift_status);
										}
										break;
									}
								}
							} else {
								$object->getActiveSheet()->setCellValueByColumnAndRow($col++, $row, '');
							}
						}

						// $month_total_ot_decimal = toDecimal($month_total_ot);
						// $month_total_ot_rd_decimal = toDecimal($month_total_ot_rd);
						// $month_total_ot_ph_decimal = toDecimal($month_total_ot_ph);

						$month_total_ot_decimal = toDecimal($r['month_overtime']);
						$month_total_ot_rd_decimal = toDecimal($r['month_overtime_rd']);
						$month_total_ot_one_point_zero_decimal = $month_total_ot_decimal + $month_total_ot_rd_decimal;
						$month_total_ot_off_decimal = toDecimal($r['month_overtime_off']);
						$month_total_ot_ph_decimal = toDecimal($r['month_overtime_ph_x3']);
						$month_total_ot_two_point_zero_decimal = $month_total_ot_off_decimal + $month_total_ot_ph_decimal;

						// Usage for your summary columns
						setNumericCell($object->getActiveSheet(), $col++, $row, $month_total_ot_decimal ?? null);
						setNumericCell($object->getActiveSheet(), $col++, $row, $month_total_ot_rd_decimal ?? null);
						setNumericCell($object->getActiveSheet(), $col++, $row, $month_total_ot_one_point_zero_decimal ?? null);
						setNumericCell($object->getActiveSheet(), $col++, $row, $month_total_ot_off_decimal ?? null);
						setNumericCell($object->getActiveSheet(), $col++, $row, $month_total_ot_ph_decimal ?? null);
						setNumericCell($object->getActiveSheet(), $col++, $row, $month_total_ot_two_point_zero_decimal ?? null);
						$object->getActiveSheet()->setCellValueByColumnAndRow($col++, $row, $r['food_allowance_days'] ?? '');
						$object->getActiveSheet()->setCellValueByColumnAndRow($col++, $row, $r['monthly_dsa_count'] ?? '');
						$object->getActiveSheet()->setCellValueByColumnAndRow($col++, $row, $r["monthly_nsa_count"] ?? '');


						// Add borders to data rows
						foreach (range(0, $col - 1) as $colIndex) {
							$object->getActiveSheet()->getStyleByColumnAndRow($colIndex, $row)
								->getBorders()
								->getAllBorders()
								->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
						}
				...
				// Freeze panes for easier navigation (freeze headers)
				$object->getActiveSheet()->freezePaneByColumnAndRow(5, $header_row + 1);


				// Save Excel file
				$file_name = "({$data["branch_name"]}) OT Summary - " . date('F Y', strtotime($first_day)) . ".xlsx";
				$safe_file_name = preg_replace('/[^a-zA-Z0-9-_\.\(\)]/', '_', $file_name);
				$new_file = "uploads/summary/" . $safe_file_name;

				// Ensure directory exists
				if (!is_dir("uploads/summary")) {
					mkdir("uploads/summary", 0777, true);
				}
				...
				$path = base_url() . $new_file;
				echo "</br><center><div style='width:40%'><a href='$path' download='$safe_file_name'><button class='btn btn-primary btn-block'>Download OT Summary - " . date('F Y', strtotime($first_day)) . "</button></a></div></center>";
			} catch (Exception $e) {
				// Comprehensive error handling
				error_log("OT Summary Export Error: " . $e->getMessage());
				echo "<div class='alert alert-danger'><strong>Error:</strong> " . htmlspecialchars($e->getMessage()) . "</div>";
				echo "<center><button class='btn btn-warning' onclick='history.back()'>Go Back</button></center>";
			}
		...
		} else if ($_POST['type'] == "mcb01_clocking") {
			// Get clocking records
			$employee_ids = array_map(function($emp) { return $emp->id; }, $employees);
			$employee_ids_str = implode(',', $employee_ids);

                $query = "SELECT
                	clockings_news.*,
                    clockings_news.address as clocking_address,
                	employees.first_name,
                	employees.last_name,
			...
			AND DATE(clockings_news.datetime) BETWEEN '$first_day' AND '$last_day'
			ORDER BY employees.first_name, employees.last_name, clockings_news.datetime";

			$clocking_data = $this->db->query($query)->result_array();

			// Group clocking data by employee and date
			$grouped_data = [];
			$total_clockings = count($clocking_data);
			$processed_clockings = 0;
			foreach ($clocking_data as $clocking) {
				$processed_clockings++;
				// Update 'Extracting Data' progress occasionally to keep UI responsive
				if ($total_clockings > 0 && ($processed_clockings % 50 == 0 || $processed_clockings == $total_clockings)) {
					$percentage_extract = floor(($processed_clockings / $total_clockings) * 100);
					echo "<script>$('#loading1 .progress-bar').css('width', '" . $percentage_extract . "%').attr('aria-valuenow', " . $percentage_extract . ").html('" . $percentage_extract . "%');</script>";
					ob_flush();
					flush();
				}
				$emp_key = $clocking['employee_id'];
				$date_key = date('Y-m-d', strtotime($clocking['datetime']));
				$key = $emp_key . '_' . $date_key;

				if (!isset($grouped_data[$key])) {
					$grouped_data[$key] = [
						'employee_name' => trim($clocking['first_name'] . ' ' . $clocking['last_name']),
						'special_id' => $clocking['special_id'],
						'address' => isset($clocking['clocking_address']) ? $clocking['clocking_address'] : '',
						'branch_name' => $clocking['branch_name'],
						'date' => $date_key,
						'shift_name' => $clocking['shift_name'],
						'clockings' => []
					];
				}

				$grouped_data[$key]['clockings'][] = [
					'type' => $clocking['type'],
					'datetime' => $clocking['datetime'],
					'mode' => $clocking['mode'],
					'scan_distance' => $clocking['scan_distance'],
					'clocking_location' => $clocking['address'] ?? '',
					'remark' => $clocking['clocking_remark'] ?? '',
					'mac_address' => $clocking['mac_address'],
					'device_location' => $clocking['device_location']
				];
				// no-op: address will be used for clocking location display
			}

			$total_records = count($grouped_data);
			...
			if ($_POST['file_type'] == "pdf") {
				...
				$html2 = $this->load->view('clocking_export_pdf', $data, true);
				...
				$object->getDefaultStyle()->applyFromArray($style);
				$name_columns = array("Branch", "From", "To", "Generated at", "Generated by");
				// Columns: Employee Name, Clock In, Mode In, Clock in location, Clock in Remark, Clock Out, Mode Out, Clock Out Location, Clock Out Remark, Total, Accumulate
				$table_columns = array(
					"Employee Name",
					"Clock In",
					"Clock In Method",
				...
				foreach ($grouped_data as $i => $group) {
					// Sort clockings by datetime
					usort($group['clockings'], function($a, $b) {
						return strtotime($a['datetime']) - strtotime($b['datetime']);
					});

					// Extract all clock in and clock out details
					$ins_with_details = [];
					$outs_with_details = [];

					foreach ($group['clockings'] as $clocking) {
						$time = date('d/m/Y H:i', strtotime($clocking['datetime']));
						$mode = isset($clocking['mode']) ? trim($clocking['mode']) : '';
						$remark = '';
						if (isset($clocking['clocking_remark'])) {
							$remark = trim($clocking['clocking_remark']);
						} elseif (isset($clocking['remark'])) {
							$remark = trim($clocking['remark']);
						}

						$location = isset($clocking['clocking_location']) ? $clocking['clocking_location'] : '';

						if (strtolower($clocking['type']) === 'in') {
							$ins_with_details[] = [
								'time' => $time,
								'mode' => $mode,
								'location' => $location,
					...
					// Compute worked total by pairing ins/outs in order
					$total_seconds = 0;
					$in_stack = [];
					foreach ($group['clockings'] as $c) {
						$t = strtotime($c['datetime']);
						if (strtolower($c['type']) === 'in') {
							$in_stack[] = $t;
						} else {
							// out
							if (!empty($in_stack)) {
								$in_time = array_shift($in_stack);
								if ($t >= $in_time) {
									$total_seconds += ($t - $in_time);
								}
							}
						}
					}
					...
					// Create a row for each clock-in/out pair
					$num_pairs = max(count($ins_with_details), count($outs_with_details));

					for ($p = 0; $p < $num_pairs; $p++) {
						// Show employee name on all rows
						$object->getActiveSheet()->setCellValueByColumnAndRow(0, $row, $group['employee_name']);

						// Get clock-in details for this pair
						$in_time = isset($ins_with_details[$p]['time']) ? $ins_with_details[$p]['time'] : '';
						$in_mode = isset($ins_with_details[$p]['mode']) ? $ins_with_details[$p]['mode'] : '';
						$in_location = isset($ins_with_details[$p]['location']) ? $ins_with_details[$p]['location'] : '';
						$in_remark = isset($ins_with_details[$p]['remark']) ? $ins_with_details[$p]['remark'] : '';

						// Get clock-out details for this pair
						$out_time = isset($outs_with_details[$p]['time']) ? $outs_with_details[$p]['time'] : '';
						$out_mode = isset($outs_with_details[$p]['mode']) ? $outs_with_details[$p]['mode'] : '';
						$out_location = isset($outs_with_details[$p]['location']) ? $outs_with_details[$p]['location'] : '';
						$out_remark = isset($outs_with_details[$p]['remark']) ? $outs_with_details[$p]['remark'] : '';

						$object->getActiveSheet()->setCellValueByColumnAndRow(1, $row, $in_time);
						$object->getActiveSheet()->setCellValueByColumnAndRow(2, $row, $in_mode);
						$object->getActiveSheet()->setCellValueByColumnAndRow(3, $row, $in_location);
						$object->getActiveSheet()->setCellValueByColumnAndRow(4, $row, $in_remark);
						$object->getActiveSheet()->setCellValueByColumnAndRow(5, $row, $out_time);
						$object->getActiveSheet()->setCellValueByColumnAndRow(6, $row, $out_mode);
						$object->getActiveSheet()->setCellValueByColumnAndRow(7, $row, $out_location);
						$object->getActiveSheet()->setCellValueByColumnAndRow(8, $row, $out_remark);

						// Show total only on last row
						if ($p === $num_pairs - 1) {
							$total_value = $total_hours_decimal ? (float) $total_hours_decimal : null;
							if ($total_value !== null) {
								$object->getActiveSheet()->setCellValueByColumnAndRow(9, $row, $total_value);
								$object->getActiveSheet()->getStyleByColumnAndRow(9, $row)
								...
								// Add to cumulative hours and display
								$cumulative_hours += $total_value;
								$object->getActiveSheet()->setCellValueByColumnAndRow(10, $row, $cumulative_hours);
								$object->getActiveSheet()->getStyleByColumnAndRow(10, $row)
									->getNumberFormat()->setFormatCode('0.0000');
								$object->getActiveSheet()->getStyleByColumnAndRow(10, $row)
									->getAlignment()
									->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT)
								...
								$object->getActiveSheet()->setCellValueByColumnAndRow(9, $row, '');
							}
						}

						// Enable wrap text for relevant cells
						foreach ([3,4,7,8] as $wrapCol) {
							$object->getActiveSheet()->getStyleByColumnAndRow($wrapCol, $row)
								->getAlignment()
								->setWrapText(true)
					...
					$percentage = floor((($i + 1) / $total_records) * 100);
				...
				if ($_POST['file_type'] == 'xlsx') {
					$file_name = "($branch_name) Clocking Records - $first_day to $last_day " . time() . ".xlsx";
					...
					$file_name = "($branch_name) Clocking Records - $first_day to $last_day " . time() . ".xls";
			...
			echo "</br> <center><div style='width:40%'><a href='" . base_url() . "exports'><button style='margin-bottom	:40px' class='btn btn-primary btn-block'>Back to Export Summary</button></a></div></center>";
			echo '</div>';
			insert_log("Simple", ["action" => "Exported,Clocking Data"]);
		} else if ($_POST['type'] == "lateness_report") {
			$all_data = array();
			$employee_count = count($employees);
			foreach ($employees as $i => $emp) {
				$data = calculate_summary_data($emp->id, $first_day, $last_day, "short", $emp, $result_list, $result_list_overnight, $company_working_hours, false, $company_ot_settings, $company_early_ot_settings, $approved_ot_list, $branch_rest_days);
			...
			if ($_POST['file_type'] == "pdf") {
				...
				// Create monthly worksheets - FIXED LOGIC
				$current_date = clone $start_date;
				$current_date->modify('first day of this month'); // Start from first day of start month

				$end_month = clone $end_date;
				$end_month->modify('first day of next month'); // Go to first day of next month after end

				$month_count = 0;

				// Create monthly tabs - iterate through all months
				while ($current_date < $end_month) {
					$month_name = $current_date->format('F Y');

					// Calculate actual start and end for this month
					$month_start_date = clone $current_date;
					$month_end_date = clone $current_date;
					$month_end_date->modify('last day of this month');

					// Adjust for actual selected date range
					...
					// Only create tab if there's data for this month
					if ($month_start_date <= $month_end_date) {
						// Create new worksheet for this month
						if ($month_count == 0) {
							$object->setActiveSheetIndex(0);
							$object->getActiveSheet()->setTitle(substr($month_name, 0, 31));
						} else {
							$object->createSheet();
						...
						// Write header for this month's tab
						foreach ($name_columns as $field) {
							$object->getActiveSheet()->setCellValueByColumnAndRow($column, 1, $field);
							$object->getActiveSheet()->getStyleByColumnAndRow($column, 1)->getFont()->setBold(true);
							$column++;
						...
						$object->getActiveSheet()->setCellValueByColumnAndRow(0, 2, $data["branch_name"]);
						$object->getActiveSheet()->setCellValueByColumnAndRow(1, 2, $month_start_date->format('d/m/Y'));
						$object->getActiveSheet()->setCellValueByColumnAndRow(2, 2, $month_end_date->format('d/m/Y'));
						$object->getActiveSheet()->setCellValueByColumnAndRow(3, 2, date("d/m/Y H:i:s"));
						$object->getActiveSheet()->setCellValueByColumnAndRow(4, 2, $current_user["first_name"]);
						...
						// Calculate data for each employee for this specific month
						foreach ($employees as $emp) {
							// Calculate data for this specific month range
							$monthly_data = calculate_summary_data(
								$emp->id,
								$month_start_date->format('Y-m-d'),
								$month_end_date->format('Y-m-d'),
								"short",
								$emp,
								$result_list,
								$result_list_overnight,
							...
							$object->getActiveSheet()->setCellValueByColumnAndRow(0, $row, $monthly_data["employee"]->first_name);
							$object->getActiveSheet()->setCellValueByColumnAndRow(1, $row, $monthly_data["employee"]->special_id);
							$object->getActiveSheet()->setCellValueByColumnAndRow(14, $row, $monthly_data["late_count"]);
							$object->getActiveSheet()->setCellValueByColumnAndRow(15, $row, $monthly_data["lateness_time_deducted"]);
							...
							$total_months = ceil(($end_date->diff($start_date)->days + 1) / 30);
							$current_progress = (($month_count * $total_employees) + $employee_counter) / ($total_months * $total_employees) * 100;
							$percentage = floor($current_progress);
				...
				// Set active sheet back to first one
				$object->setActiveSheetIndex(0);
				...
				if ($_POST['file_type'] == 'xlsx') {
					...
					$file_name = "($branch_name) Lateness Summary - $first_day to $last_day " . time() . ".xls";
			...
			redirect($path);
		} else if ($_POST['type'] == "tsf01_csv_report") {
			$all_data = array();
			$employee_count = count($employees);
			...
			// Only generate this single file
			$file = $this->tsf01ReportCsv($style, $all_data, $date2, $branch_name, $first_day, $last_day);
			$download_path = base_url() . $file;
			...
			echo '</br> <br> <b>Export Completed</b> </br>';
			echo "</br> <center><div style='width:40%'><a href='$download_path'><button class='btn btn-primary btn-block'>Download File</button></a></div></center>";
			echo "</br> <center><div style='width:40%'><a href='" . base_url() . "exports'><button style='margin-bottom:40px' class='btn btn-primary btn-block'>Back to Export Summary</button></a></div></center>";
			echo '</div>';

			insert_log("Simple", ["action" => "Exported,TSF01 Report)"]);
			...
			foreach ($employees as $i => $emp) {
				...
				if ($_POST['file_type'] == 'pdf') {
					...
					foreach ($data['dates'] as $d) {
						...
						if (!in_array($d->date, $data["public_holidays"]) && !in_array($d->day_name, $data["rest_days"]) && $d->is_shift == 'true' && !$d->is_replaced_ph && !$d->is_employee_off_day) {
			...
			foreach ($employees as $i => $emp) {
				...
				ob_flush();
				flush();
				$data['type'] = "mm01_report_pdf";
				$all_data[] = $data;
			}
			$data["all_data"] = $all_data;
			if ($_POST["file_type"] === "pdf") {
				$summary_body .= $this->load->view('mm01_report_pdf', $data, true);
				...
				// Format Dates
				$monthYear = strtoupper($date1->format('M Y'));
				...
				// === Left Section ===
				// Company Name
				$sheet->setCellValue("A$row", "Mathevon Malaysia Sdn Bhd");
				$sheet->mergeCells("A$row:C$row");
				$sheet->getStyle("A$row")->getFont()->setBold(true)->setSize(8);
				$sheet->getStyle("A$row")->getFont()->setName('Arial');

				$row++;

				// Title
				$sheet->setCellValue("A$row", "Standard Hour VS OT Hour");
				$sheet->mergeCells("A$row:C$row");
				$sheet->getStyle("A$row")->getFont()->setBold(true)->setSize(8);
				$sheet->getStyle("A$row")->getFont()->getColor()->setRGB('FF0000'); // Red
				$sheet->getStyle("A$row")->getFont()->setName('Arial');


				// === Right Section ===
				$row = 1; // reset to row 1
				$rightStartCol = 'G'; // adjust as needed
				$sheet->setCellValue("{$rightStartCol}{$row}", "Month");
				$sheet->setCellValue(chr(ord($rightStartCol) + 1) . $row, "From");
				$sheet->setCellValue(chr(ord($rightStartCol) + 2) . $row, "To");

				// Header styling
				foreach (range(0, 2) as $i) {
					$cell = chr(ord($rightStartCol) + $i) . $row;
				...
				$row++;
				$sheet->setCellValue("{$rightStartCol}{$row}", $monthYear);
				$sheet->getStyle("G$row")->getFont()->setBold(true); // Bold Date
				$sheet->setCellValue(chr(ord($rightStartCol) + 1) . $row, $all_data[0]['from_f']);
				$sheet->setCellValue(chr(ord($rightStartCol) + 2) . $row, $all_data[0]['to_f']);
				...
				foreach ($headers as $header) {
					$cell = PHPExcel_Cell::stringFromColumnIndex($column) . '4';
					$sheet->setCellValue($cell, $header);
					$sheet->getStyle($cell)->getFont()->setBold(true);
					$sheet->getStyle($cell)->applyFromArray($styleArray);

					// Apply header background color
					$sheet->getStyle($cell)->applyFromArray($headerBgColor);
					$column++;
				}
				...
				foreach ($all_data as $r) {
					...
					foreach ($r['dates'] as $d) {
						...
						$in_time = reset($d->in_outs) ?? '';
						$out_time = end($d->in_outs) ?? '';

						$sheet->setCellValueByColumnAndRow(0, $row, $r["employee"]->special_id); // EMP ID
						$sheet->setCellValueByColumnAndRow(1, $row, $r["employee"]->first_name); // EMP Name
						$sheet->setCellValueByColumnAndRow(2, $row, date('d-m-Y (D)', strtotime($d->date))); // Date
						$sheet->getStyle("C$row")->getFont()->setBold(true); // Bold Date

						// Shift
						$sheet->setCellValueByColumnAndRow(3, $row, $clock->code ?? '');
						$sheet->getStyle("D$row")->applyFromArray($grayBlueBg);

						// In/Out
						$sheet->setCellValueByColumnAndRow(4, $row, !empty($in_time) ? $in_time : '');
						$sheet->setCellValueByColumnAndRow(5, $row, !empty($out_time) ? $out_time : '');
						$sheet->getStyle("E$row")->applyFromArray($grayBlueBg);
						$sheet->getStyle("E$row")->getFont()->setBold(true);
						$sheet->getStyle("F$row")->applyFromArray($grayBlueBg);
						$sheet->getStyle("F$row")->getFont()->setBold(true);

						// Actual Hours
						$sheet->setCellValueByColumnAndRow(6, $row, $d->work_hours);

						// Production Cell (blank)
						$sheet->setCellValueByColumnAndRow(7, $row, $r["employee"]->section);
						$sheet->getStyle("H$row")->applyFromArray($lightBlueBg);

						// Standard Hour
						$sheet->setCellValueByColumnAndRow(8, $row, $d->employee_shift_hours);
						$sheet->getStyle("I$row")->applyFromArray($lightBlueBg);

						// OT Hour
						$sheet->setCellValueByColumnAndRow(9, $row, $overtime);
						$sheet->getStyle("J$row")->applyFromArray($lightBlueBg);
						...
						$total_overtime = add_time($overtime, $total_overtime);
						$row++;
					}
				}
				...
				// Totals Row
				$sheet->setCellValueByColumnAndRow(6, $row, $total_work_hours);
				$sheet->setCellValueByColumnAndRow(8, $row, $total_standard_hours);
				$sheet->setCellValueByColumnAndRow(9, $row, $total_overtime);

				$columns = ['G', 'I', 'J'];
				foreach ($columns as $col) {
					$sheet->getStyle("{$col}{$row}")->applyFromArray([
						'borders' => ['top' => ['style' => PHPExcel_Style_Border::BORDER_DOUBLE]],
						'font' => ['bold' => true],
					]);
				}

				$row += 3; // Leave some space after totals

				// Table Header Row
				$sheet->setCellValue("H$row", "Production Cell");
				$sheet->setCellValue("I$row", "Summary " . date('M Y', strtotime($first_day)));
				...
				// Column Headings
				$sheet->setCellValue("I$row", "Standard Hour");
				$sheet->setCellValue("J$row", "OT Hour");
				...
				foreach ($summary_data as $item) {
					$sheet->setCellValue("H$row", $item['cell']);
					$sheet->setCellValue("I$row", $item['standard']);
					$sheet->setCellValue("J$row", $item['ot']);
				...
				// Determine file type and writer
				$file_type = $_POST['file_type'] ?? 'xlsx'; // Default to xlsx if not set
				...
				$writer->save("uploads/summary/$file_name");
		...
		} else if ($_POST['type'] == "alya01_custom_report") {
			if (!in_array((int)$cid, companies_allowed_for_alya01_custom_report())) {
				show_error('This report is not available for your company.');
				return;
			}

			require_once APPPATH . 'libraries/Alya01CustomReportService.php';
			$alya01_service = new Alya01CustomReportService();

			try {
				$result = $alya01_service->generate([
					'employees' => $employees,
					'first_day' => $first_day,
					'last_day' => $last_day,
			...
			echo "</br> <center><div style='width:40%'><a href='" . base_url() . "exports'><button style='margin-bottom	:40px' class='btn btn-primary btn-block'>Back to Export Summary</button></a></div></center>";
			echo '</div>';
			insert_log("Simple", ["action" => "Exported,ALYA01 Custom Report"]);
	...
	function excel($id, $first_day, $last_day)
	{
		try {
			...
			if (!$date1) {
				$date1 = DateTime::createFromFormat('d/m/Y', $first_day);
			}
			$date2 = DateTime::createFromFormat('Y-m-d', $last_day);
			if (!$date2) {
				$date2 = DateTime::createFromFormat('d/m/Y', $last_day);
			}

			$first_day_formatted = $date1->format('Y-m-d');
			$last_day_formatted = $date2->format('Y-m-d');

			$data = calculate_summary_data($id, $first_day_formatted, $last_day_formatted);
			$data['from_f'] = $date1->format('d/m/Y');
			$data['to_f'] = $date2->format('d/m/Y');
			...
			header('Content-Type: application/vnd.ms-excel');
			header('Content-Disposition: attachment;filename="' . htmlspecialchars($data['employee']->first_name . " " . $data['employee']->special_id . " " . $data["month_name"] . " - Summary.xls", ENT_QUOTES) . '"');
			header('Content-Transfer-Encoding: binary');
	...
	public function get_sql_xcrud()
	{
		...
		if (in_array($cid, [152, 215, 206, 229]) || in_array($cid, companies_allowed_for_meal_allowance())) {
		...
		if (in_array($cid, companies_allowed_for_shift_allowance())) {
			$xcrud->label(array('sql_nsa_name' => 'Name', 'sql_nsa_code' => 'Code', 'sql_nsa_description' => 'Description', 'sql_nsa_rate' => 'Rate'));
			$xcrud->label(array('sql_dsa_name' => 'Name', 'sql_dsa_code' => 'Code', 'sql_dsa_description' => 'Description', 'sql_dsa_rate' => 'Rate'));
		}
		...
		if (in_array($cid, [152, 215, 206, 229]) || in_array($cid, companies_allowed_for_meal_allowance())) {
			...
			} else if (in_array($cid, companies_allowed_for_meal_allowance())) {
				$tabName = 'Meal Allowance';
			} else if ($cid == 152) {
		...
		if (in_array($cid, companies_allowed_for_shift_allowance())) {
			$tabName = 'Day Shift Allowance';
			$xcrud->fields('sql_dsa_name, sql_dsa_code, sql_dsa_description, sql_dsa_rate', false, $tabName);
	...
	public function pendingUnpaidLeavesLogFile($style, $unpaid_leaves_absent_days, $cid, $date2, $branch_name, $first_day, $last_day)
	{
		...
		foreach ($unpaid_leaves_absent_days as $key => $dated_array) {
			foreach ($dated_array as $unpaid_leave) {
				if ($unpaid_leave["type"] == "absent" && ($cid == 153 || $cid == 255)) {
					continue;
				}
	...
	public function tsf01ReportCsv($style, $all_data, $date2, $branch_name, $first_day, $last_day)
	{
		$worked_shift_days_object = new PHPExcel();

		$worked_shift_days_object->setActiveSheetIndex(0);
		$worked_shift_days_object->getDefaultStyle()->applyFromArray($style);
		$table_columns = array("EmpCodeC", "DeptCode", "Name", "DeptNameC", "ClockDateD", "PostDate", "Normal", "OT1.5", "HW15", "OT2.0", "HW20", "OT3.0", "HW30", "Meal", "OTA8", "GroupCodeC", "Comments");
		...
		foreach ($all_data as $r) {
			...
			foreach ($r["dates"] as $d) {
				...
				// Classify the OT day
				$is_rd = in_array($d->day_name, $rest_days);
				$is_ph = in_array($d->date, $public_holidays);
				$is_normal = !$is_ph && !$is_rd;
				...
				if ($is_normal) {
					$ot_15 = $d->overtime ?? "00:00";
				} elseif ($is_ph) {
					$ot_ph = $d->overtime ?? "00:00";
				} elseif ($is_rd) {
					$ot_rd = $d->overtime ?? "00:00";
				}

				// Format the date
				$date = DateTime::createFromFormat('Y-m-d', $d->date);
				$d->date = $date ? $date->format('d/m/Y') : $d->date;

				// Write to Excel: one row per OT day
				$worked_shift_days_object->getActiveSheet()->setCellValueByColumnAndRow(0, $row, $r["employee"]->special_id);
				$worked_shift_days_object->getActiveSheet()->setCellValueByColumnAndRow(1, $row, $r["employee"]->department);
				$worked_shift_days_object->getActiveSheet()->setCellValueByColumnAndRow(2, $row, $r["employee"]->first_name);
				$worked_shift_days_object->getActiveSheet()->setCellValueByColumnAndRow(3, $row, $r["employee"]->department);
				$worked_shift_days_object->getActiveSheet()->setCellValueByColumnAndRow(4, $row, $d->date);
				$worked_shift_days_object->getActiveSheet()->setCellValueByColumnAndRow(5, $row, $date2->format('d/m/Y'));
				$worked_shift_days_object->getActiveSheet()->setCellValueByColumnAndRow(6, $row, toDecimal($d->employee_shift_hours));
				$worked_shift_days_object->getActiveSheet()->setCellValueByColumnAndRow(7, $row, toDecimal($ot_15));
				$worked_shift_days_object->getActiveSheet()->setCellValueByColumnAndRow(8, $row, 'HW15');
				$worked_shift_days_object->getActiveSheet()->setCellValueByColumnAndRow(9, $row, toDecimal($ot_rd));
				$worked_shift_days_object->getActiveSheet()->setCellValueByColumnAndRow(10, $row, 'HW20');
				$worked_shift_days_object->getActiveSheet()->setCellValueByColumnAndRow(11, $row, toDecimal($ot_ph));
				$worked_shift_days_object->getActiveSheet()->setCellValueByColumnAndRow(12, $row, 'TBC');
				$worked_shift_days_object->getActiveSheet()->setCellValueByColumnAndRow(13, $row, $d->meal_days);
				$worked_shift_days_object->getActiveSheet()->setCellValueByColumnAndRow(14, $row, 'OTA8');
				$worked_shift_days_object->getActiveSheet()->setCellValueByColumnAndRow(15, $row, '');
				$worked_shift_days_object->getActiveSheet()->setCellValueByColumnAndRow(16, $row, $remark);
		...
		$file_name = "($branch_name) tsf01-report-csv - $first_day to $last_day " . time();

		$object_writer = PHPExcel_IOFactory::createWriter($worked_shift_days_object, 'CSV');
		header('Content-Type: application/vnd.ms-excel');
		header('Content-Disposition: attachment;filename="' . $file_name . '.csv"');
		//$object_writer->save('php://output');
		$new_file = "uploads/summary/tsf01-report-csv-" . time() . ".csv";
		$object_writer->save($new_file);
	...
	public function pendingAllowanceLogFile($style, $all_data, $cid, $date2, $branch_name, $first_day, $last_day)
	{
		...
		if ($cid == 215) {
			...
			foreach ($all_data as $r) {
				...
				$total_allowance = $r["employee"]->ta_rate ?? 100;
			...
			foreach ($all_data as $r) {
				$sql_data_of_emp = get_sql_data($r["employee"]->branch_id);
				$ln01_waived_days = $r["ln01_waived_days"];
				$ln01_attendance_allowance_days = $r["ln01_attendance_allowance_days"];
				$worked_days = $ln01_attendance_allowance_days + $r["worked_rest_days"] - $ln01_waived_days;

				$amount = $worked_days * $r["employee"]->aa_rate;

				if ($worked_days > 0 && $amount > 0) {
					$object->getActiveSheet()->setCellValueByColumnAndRow(0, $row, $date2->format('d/m/Y'));
					$object->getActiveSheet()->setCellValueByColumnAndRow(1, $row, $date2->format('d/m/Y'));
					$object->getActiveSheet()->setCellValueByColumnAndRow(2, $row, $r["employee"]->special_id);
					$object->getActiveSheet()->setCellValueByColumnAndRow(3, $row, $sql_data_of_emp["sql_aa_code"]);
		...
		if (in_array($cid, companies_allowed_for_shift_allowance())) {
			foreach ($all_data as $r) {
				$sql_data_of_emp = get_sql_data($r["employee"]->branch_id);
				$day_shifts = $r["monthly_dsa_count"];
				$night_shifts = $r["monthly_nsa_count"];
				if ($day_shifts) {
					$object->getActiveSheet()->setCellValueByColumnAndRow(0, $row, $date2->format('d/m/Y'));
					$object->getActiveSheet()->setCellValueByColumnAndRow(1, $row, $date2->format('d/m/Y'));
					$object->getActiveSheet()->setCellValueByColumnAndRow(2, $row, $r["employee"]->special_id);
					$object->getActiveSheet()->setCellValueByColumnAndRow(3, $row, $sql_data_of_emp["sql_dsa_code"]);
					$object->getActiveSheet()->setCellValueByColumnAndRow(4, $row, $sql_data_of_emp["sql_dsa_description"]);
					$object->getActiveSheet()->setCellValueByColumnAndRow(5, $row, $day_shifts);
					$object->getActiveSheet()->setCellValueByColumnAndRow(6, $row, $r["employee"]->dsa_rate);
					$object->getActiveSheet()->setCellValueByColumnAndRow(7, $row, $day_shifts * $r["employee"]->dsa_rate);
