...
class Exports extends CI_Controller
{
	...
	public function summary_pdf()
	{
		...
		if ($_POST['type'] == "accounts") {
			...
			$output = $this->dompdf->output();
			$file_name = "($branch_name) MM01 Report - $first_day to $last_day.pdf";
			$new_file = "uploads/summary/" . $file_name;
			...
			echo '</div>';
			// redirect($path);
		} elseif ($_POST["type"] === "mm01_report_excel") {
			$all_data = array();
			$summary_body = '';
			$employee_count = count($employees);
			...
			foreach ($all_data as $r) {
				foreach ($r['dates'] as $d) {
					...
					$total_work_hours += $d->work_hours;
					$total_standard_hours += $d->employee_shift_hours;
					$total_overtime += $d->overtime;
					$row++;
				}
			}
			...
			// Totals Row
			$sheet->setCellValueByColumnAndRow(6, $row, number_format($total_work_hours, 2));
			$sheet->setCellValueByColumnAndRow(8, $row, number_format($total_standard_hours, 2));
			$sheet->setCellValueByColumnAndRow(9, $row, number_format($total_overtime, 2));
			...
			$file_name = 'MM01_Report_' . date('Ymd_His') . '.xlsx';
