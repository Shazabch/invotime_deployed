...
class Exports extends CI_Controller
{
	...
	public function summary_pdf()
	{
		...
		if ($_POST['type'] == "accounts") {
			...
			if ($_POST["file_type"] === "pdf") {
				...
				$headers = [
					'EMP ID', 'EMP Name', 'Date', 'Shift', 'In', 'Out',
					'Actual Hours', 'Production Cell', 'Standard Hour', 'OT Hour'
				];

				$column = 0;
				foreach ($headers as $header) {
					$cell = PHPExcel_Cell::stringFromColumnIndex($column) . '1';
	...
	public function tsf01ReportCsv($style, $all_data, $date2, $branch_name, $first_day, $last_day)
	{
		...
		$worked_shift_days_object->setActiveSheetIndex(0);
		$worked_shift_days_object->getDefaultStyle()->applyFromArray($style);
		$table_columns = array("EmpCodeC", "DeptCode", "Name", "DeptNameC", "ClockDateD", "PostDate", "Normal", "OT1.5", "HW15", "OT2.0", "HW20", "OT3.0", "HW30", "Meal", "OTA8", "GroupCodeC", "Comments", "Meal allow");
		...
		foreach ($all_data as $r) {
			...
			foreach ($r["dates"] as $d) {
				...
				$worked_shift_days_object->getActiveSheet()->setCellValueByColumnAndRow(17, $row, '');
