...
class Late_break_days extends CI_Controller
{
    ...
    function index()
    {
        if (!is_page_permitted('late_break_days')) {
            redirect_if_not_permitted();
        }

        $current_user = get_user();
        $data['pageTitle'] = "Late (Break) Sheet";
        $data['active_menu'] = "late_break_days";
        $this->load->view('header', $data);
        $data["menus"] = get_menus();
        $this->load->view('sidebar', $data);
        $data["filters_form_action"] = "late_break_days";
        ...
        if ($permissions_level == "Outlet") {
            ...
            if (empty($this->input->get("branch")) || $this->input->get("branch") != $bid) {
                redirect("late_break_days?branch=$bid&" . getDateRangeFilterURLString($current_user['start_day']));
                return;
            }
        }
        $where_filter = "";
        ...
        $result = $this->db->query("SELECT e.id, special_id,first_name,is_ot,is_early_ot, ot_type, ot_round, early_ot_round, branch_id, ignore_breaks_after_endtime FROM employees e LEFT JOIN branches b on b.id = e.branch_id  INNER JOIN roles r ON e.role_id = r.id LEFT JOIN employee_groups_relation egr ON egr.employee_id = e.id where $where_filter AND e.deleted_at is NULL AND employee_status = 'active' AND r.exclude_from_system = 'no'
			GROUP BY e.id, special_id, first_name, is_ot, is_early_ot, ot_type, ot_round, early_ot_round, branch_id, ignore_breaks_after_endtime ORDER BY special_id LIMIT $skip,$limit")->result();
        ...
        $check_list = $this->db->select('employee_id, is_late_break, late_break_date as date')->from('late_break_days')->where_in('employee_id', $employees_ids)->where('late_break_date >=', $first_day)->where('late_break_date <=', $last_day)->get()->result();
        ...
        $manual_late_break_list_all = get_manual_late_break_list_all($employees_ids, $first_day, $last_day);
        ...
        foreach ($result as $emp) {
            ...
            $manual_late_break_list = get_manual_late_break_list_by_id($manual_late_break_list_all, $emp->id);

			$late_break_round_off_settings = [];

            $shift_list = $this->db->select('overnight,date,half_day')->from('shift_days sd')->join('shifts s', 's.id = sd.shift_id')->where('FIND_IN_SET(' . $emp->id . ',employees)>', 0)->where('date >=', $first_day)->where('date <=', $last_day)->get()->result();
            ...
            foreach ($data['period_of_dates'] as $periodDate) {
                ...
                $check = true;
                $check_is_late_break = $this->search_from_list_by_id($check_list, $temp1['day'], $emp->id);
                if ($check_is_late_break) {
                    $check = $check_is_late_break->is_late_break == "Y" ? true : false;
                }

                $temp1['is_late_break'] = $check ? true : false;
				...
				$late_break_time = count_late_break($temp1['day'], $result, $overnight, $half_day, $manual_late_break_list, $is_shift, $emp->ignore_breaks_after_endtime);
				if(isset($late_break_round_off_settings[$emp->branch_id])){
					$late_break_round_off = $late_break_round_off_settings[$emp->branch_id];
				}else{
					$late_break_round_off = $this->db->select('start, end, round_to')->from('late_break_round_settings')->where('branch_id', $emp->branch_id)->get()->result();
					$late_break_round_off_settings[$emp->branch_id] = $late_break_round_off;
				}

				$late_break_time = round_off_late_break($late_break_time, $late_break_round_off, false);

				$late_break_time = ($late_break_time == "" || $late_break_time == "00:00") ? "-" : $late_break_time;

				$temp1['late_break_time'] = $late_break_time;

                $days_wise[] = $temp1;
            }

            $temp["late_break_data"] = $days_wise;
        ...
        $this->load->view('late_break_days', $data);
    ...
    function change_status()
    {
        ...
        $late_break_date = $request['day'];

        $is_late_break = ($request['is_late_break'] == 1) ? 'Y' : 'N';



        $data = array('employee_id' => $employee_id,

            'late_break_date' => $late_break_date,

            'is_late_break' => $is_late_break);

        $this->db->replace('late_break_days', $data);
    }

    function approve_all_late_break()
    {
        ...
        foreach ($period as $date) {
            $late_break_date = $date->format('Y-m-d');
            $data = array('employee_id' => $employee_id,
                'late_break_date' => $late_break_date,
                'is_late_break' => 'Y');
            $this->db->replace('late_break_days', $data);
