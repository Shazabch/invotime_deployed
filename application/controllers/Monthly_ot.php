...
class Monthly_ot extends CI_Controller
{
    ...
    public function monthly_view($id = 0, $dep = false)
    {
        if (!is_page_permitted('monthly_view')) {
        ...
        if ($permissions_level == "Outlet") {
            ...
            if (empty($this->input->get("branch")) || $this->input->get("branch") != $bid) {
                redirect("monthly_ot/monthly_view?branch=$bid&month=" . date('m') . "&year=" . date('Y'));
                return;
            }
        }
        ...
        $data["employees_dropdown"] = $this->db->query("SELECT employees.id, special_id,first_name,last_name, employees.branch_id FROM employees INNER JOIN roles ON employees.role_id = roles.id WHERE employees.deleted_at IS NULL 
            AND (
        ...
        if (!$data['employee']) {
            redirect('monthly_ot/monthly_view/' . $data["employees_dropdown"][0]->id);
            die();
        }
        ...
        if (empty($_GET)) {
            ...
            $first_day = $date1->format('Y-m-01');
            $last_day = $date1->format('Y-m-t');
        }
        ...
        $data['to_p'] = $last_day;
        $data['emp_id'] = $id;
        $data['pageTitle'] = "Monthly OT";
        $data['active_menu'] = "monthly_ot/monthly_view/";
        $this->load->view('header', $data);
        $data["menus"] = get_menus();

        $data["branches"] = $this->db->query("SELECT * FROM branches WHERE company_id = $cid  $branch_where_filter ORDER BY name")->result();

        $this->load->view('sidebar', $data);
        $this->load->view('monthly_ot/view', $data);
