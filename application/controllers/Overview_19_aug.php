...
class Overview extends CI_Controller
{
    ...
    ///////////////////////////////
        ...
        $data["gender_breakdown"] = $this->db->select('sex, count(*) as gender_count')->from('employees e')->join('roles r', 'e.role_id = r.id')->where("e.company_id = $cid $where_branch_1 ")->where('exclude_from_system', 'no')->where('e.deleted_at is null')->where('employee_status', 'active')->group_by('sex')->get()->result();
    ...
    // die();
        ...
        $data["departments_breakdown"] = $this->db->select('d.name, count(*) as count')->from('employees e')->join('roles r', 'e.role_id = r.id')->join('departments d', 'e.department_id = d.id')->where("e.company_id = $cid $where_branch_1 ")->where('employee_status', 'active')->where('exclude_from_system', 'no')->where('e.deleted_at is null')->group_by('department_id')->order_by('count DESC')->get()->result();




        if (!isset($branch)) {
            $data["outlets_breakdown"] = $this->db->select('b.name, count(*) as count')->from('employees e')->join('roles r', 'e.role_id = r.id')->join('branches b', 'e.branch_id = b.id')->where("e.company_id = $cid $where_branch_1 ")->where('employee_status', 'active')->where('exclude_from_system', 'no')->where('e.deleted_at is null')->group_by('branch_id')->order_by('count DESC')->get()->result();
        }
        ...
        $data["late_today_count"] = $this->db->query("SELECT COUNT(id) AS cnt FROM 
        ...
        shifts.grace_time, shifts.start_time, shifts.name as shift_name, clockings_news.*
            ...
            WHERE clockings_news.type='in' AND employees.company_id = $cid
            $where_branch_2 
            AND clockings_news.datetime > '$from_today'
            GROUP BY employees.id,
                    DATE(datetime)
            HAVING DATE_FORMAT(clockings_news.datetime, '%H:%i') > DATE_FORMAT(shifts.grace_time, '%H:%i')) AS tbl_temp")->row();





        $data["early_today_count"] = $this->db->query("SELECT COUNT(id) AS cnt FROM 
        (SELECT employees.first_name,employees.special_id,
        branches.name as branch_name,departments.name as department_name,
        shifts.grace_time, shifts.start_time, shifts.name as shift_name, clockings.*
            ...
            WHERE clockings.type='in' AND employees.company_id = $cid
            $where_branch_2 
            AND clockings.datetime > '$from_today'
            GROUP BY employees.id,
                    DATE(datetime)
            HAVING DATE_FORMAT(clockings.datetime, '%H:%i') < DATE_FORMAT(shifts.start_time, '%H:%i')) AS tbl_temp")->row();




            $data["ontime_today_count"] = $this->db->query("SELECT COUNT(id) AS cnt FROM 
            (SELECT employees.first_name,employees.special_id,
            branches.name as branch_name,departments.name as department_name,
            shifts.grace_time, shifts.start_time, shifts.name as shift_name, clockings.*
                FROM clockings_news as clockings
                ...
                GROUP BY employees.id,
                        DATE(datetime)
                HAVING (DATE_FORMAT(clockings.datetime, '%H:%i') > DATE_FORMAT(shifts.start_time, '%H:%i') AND DATE_FORMAT(clockings.datetime, '%H:%i') < DATE_FORMAT(shifts.grace_time, '%H:%i') )  ) AS tbl_temp")->row();
            ...
            $data["onleave_today_count"] = $this->db->query("SELECT SUM(LENGTH(shift_days.employees) - LENGTH(REPLACE(shift_days.employees, ',', '')) + 1) as cnt FROM shifts INNER JOIN shift_days ON shifts.id = shift_days.shift_id where shifts.company_id = $cid $where_branch_3  AND shifts.is_leave = 'yes' AND shift_days.date = CURRENT_DATE")->row();

            $data["absent_today_count"] = $this->db->query("SELECT ((SELECT SUM(LENGTH(shift_days.employees) - LENGTH(REPLACE(shift_days.employees, ',', '')) + 1) as cnt FROM shifts INNER JOIN shift_days ON shifts.id = shift_days.shift_id where company_id = $cid $where_branch_3 AND shifts.is_leave = 'no' AND shift_days.date = CURRENT_DATE) - (SELECT COUNT(DISTINCT(clockings_news.employee_id)) FROM clockings_news INNER JOIN employees ON clockings_news.employee_id = employees.id WHERE company_id = $cid $where_branch_3 AND DATE(clockings_news.datetime) = CURRENT_DATE)) as cnt")->row();

            // hide by UMAR ---end---



            //  $attendance_pie_temp = array($data["late_today_count"]->cnt,
            //  $data["early_today_count"]->cnt,
            //  $data["ontime_today_count"]->cnt,
            //  $data["onleave_today_count"]->cnt,
            //  $data["absent_today_count"]->cnt
            // );

            //  rsort($attendance_pie_temp);

            //  $data["attendance_pie"] = $attendance_pie_temp;
        //die($this->db->last_query());
 
    
        
 
        // $data["gantt_chart_department_shift"] = gantt_chart_department_shift($is_branch, $branch_id);
        $data["gantt_chart_department_shift"] = [];


        // die("SELECT department_name,
        ...
        // INNER JOIN employees on clockings.employee_id = employees.id
        // INNER JOIN departments on employees.department_id = departments.id
        // WHERE DATE(clockings.datetime) = CURRENT_DATE AND departments.company_id = $cid $where_branch_2 AND shift_days.shift_id = clockings.shift_id
        ...
        // hide by UMAR ---start---
        // $data["attendance_by_department_today"] = $this->db->query("SELECT department_name, 

        //     SUM(case when status = 'early' then 1 else 0 end) as early,
        //     SUM(case when status = 'late' then 1 else 0 end) as late,
        //     SUM(case when status = 'ontime' then 1 else 0 end) as ontime

        //     FROM (SELECT 'early' as status, departments.name as department_name, clockings.*,shifts.grace_time, shifts.start_time FROM clockings_news as clockings
        //     INNER JOIN shift_days ON DATE(clockings.datetime) = shift_days.date 
        //     INNER JOIN shifts ON clockings.shift_id = shifts.id
        //     INNER JOIN employees on clockings.employee_id = employees.id
        //     INNER JOIN departments on employees.department_id = departments.id
        //     WHERE   clockings.type='in' AND  clockings.datetime > '$from_today' AND departments.company_id = $cid $where_branch_2 AND shift_days.shift_id = clockings.shift_id
        //     GROUP BY employees.id,department_id,DATE(datetime) HAVING DATE_FORMAT(clockings.datetime,'%H:%i') < DATE_FORMAT(shifts.start_time,'%H:%i')

        //     UNION ALL 

        //     SELECT 'late' as status, departments.name as department_name, clockings.*, shifts.grace_time,shifts.start_time FROM clockings_news as clockings
        //     INNER JOIN shift_days ON DATE(clockings.datetime) = shift_days.date 
        //     INNER JOIN shifts ON clockings.shift_id = shifts.id
        //     INNER JOIN employees on clockings.employee_id = employees.id
        //     INNER JOIN departments on employees.department_id = departments.id
        ...
        //     SELECT 'ontime' as status, departments.name as department_name, clockings.*, shifts.grace_time, shifts.start_time FROM clockings_news as clockings
        //     INNER JOIN shift_days ON DATE(clockings.datetime) = shift_days.date 
        //     INNER JOIN shifts ON clockings.shift_id = shifts.id
        //     INNER JOIN employees on clockings.employee_id = employees.id
        //     INNER JOIN departments on employees.department_id = departments.id
        //     WHERE  clockings.type='in' AND  clockings.datetime > '$from_today' AND departments.company_id = $cid $where_branch_2 AND shift_days.shift_id = clockings.shift_id
        //     GROUP BY employees.id, department_id,DATE(datetime) HAVING DATE_FORMAT(clockings.datetime,'%H:%i') >= DATE_FORMAT(shifts.start_time,'%H:%i') AND DATE_FORMAT(clockings.datetime,'%H:%i') <= DATE_FORMAT(shifts.grace_time,'%H:%i')
        //          ) as xx GROUP BY department_name")->result();
        $data["attendance_by_department_today"] = [];
            // hide by UMAR ---end---


            // var_dump($data["chart1_data"]);
            // die();
...
//CURRENT_DATE()
    ...
    // if($cid != 1){
        ...
        $data["chart1_data"] = array();
      // $this->db->query("SELECT department_name,
      ...
      //       AND MONTH(clockings.clock_in) = MONTH(CURRENT_DATE()) AND YEAR(clockings.clock_in) = YEAR(CURRENT_DATE())
      //       GROUP BY department_id,employees.id,DATE(clock_in) HAVING DATE_FORMAT(clockings.clock_in,'%H:%i') < DATE_FORMAT(shifts.start_time,'%H:%i')
      ...
      //       WHERE departments.company_id = $cid AND $branch_where_clause shift_days.shift_id = clockings.shift_id
      //       AND MONTH(clockings.clock_in) = MONTH(CURRENT_DATE()) AND YEAR(clockings.clock_in) = YEAR(CURRENT_DATE())
      //       GROUP BY department_id,employees.id,DATE(clock_in) HAVING DATE_FORMAT(clockings.clock_in,'%H:%i') > DATE_FORMAT(shifts.grace_time,'%H:%i')
      ...
      //       WHERE departments.company_id = $cid AND $branch_where_clause shift_days.shift_id = clockings.shift_id
      //       AND MONTH(clockings.clock_in) = MONTH(CURRENT_DATE()) AND YEAR(clockings.clock_in) = YEAR(CURRENT_DATE())
      //       GROUP BY department_id,employees.id,DATE(clock_in) HAVING DATE_FORMAT(clockings.clock_in,'%H:%i') >= DATE_FORMAT(shifts.start_time,'%H:%i') AND DATE_FORMAT(clockings.clock_in,'%H:%i') <= DATE_FORMAT(shifts.grace_time,'%H:%i')
      //            ) as xx GROUP BY department_name")->result();



        $data["chart2_data"] = array();
        // $this->db->query("SELECT departments.name as department_name, IFNULL(SUM(TIME_FORMAT(TIMEDIFF(a.`clock_out`, a.`clock_in` ),'%H')), 0) AS hours FROM `clockings` a INNER JOIN employees ON a.employee_id = employees.id INNER JOIN departments ON employees.department_id = departments.id WHERE $branch_where_clause clock_out IS NOT NULL AND departments.company_id = $cid AND MONTH(a.clock_in) = MONTH(CURRENT_DATE()) AND YEAR(a.clock_in) = YEAR(CURRENT_DATE()) GROUP BY department_name")->result();



        $data["chart3_data"] = array();
         // $this->db->query("SELECT department_name,
         ...
         //     SELECT 'male' as status, departments.name as department_name, clockings.*, employees.sex FROM clockings INNER JOIN employees on clockings.employee_id = employees.id INNER JOIN departments on employees.department_id = departments.id WHERE $branch_where_clause departments.company_id = $cid
         //     AND MONTH(clockings.clock_in) = MONTH(CURRENT_DATE()) AND YEAR(clockings.clock_in) = YEAR(CURRENT_DATE())

         //    GROUP BY department_id,employees.id,employees.sex,DATE(clock_in) HAVING employees.sex = 'Male'

         //    UNION ALL

         //    SELECT 'female' as status, departments.name as department_name, clockings.*,employees.sex FROM clockings INNER JOIN employees on clockings.employee_id = employees.id INNER JOIN departments on employees.department_id = departments.id WHERE $branch_where_clause departments.company_id = $cid
         //    AND MONTH(clockings.clock_in) = MONTH(CURRENT_DATE()) AND YEAR(clockings.clock_in) = YEAR(CURRENT_DATE())

         //    GROUP BY department_id,employees.id,employees.sex,DATE(clock_in) HAVING employees.sex = 'Female'
        ...
        $data["chart4_data"] = array();
        // $this->db->query("SELECT sex, COUNT(1) as count FROM(SELECT employees.sex, clockings.* FROM clockings INNER JOIN employees on clockings.employee_id = employees.id WHERE $branch_where_clause employees.company_id = $cid AND MONTH(clockings.clock_in) = MONTH(CURRENT_DATE()) AND YEAR(clockings.clock_in) = YEAR(CURRENT_DATE()) GROUP BY employees.sex,employees.id,DATE(clock_in)) as xx GROUP BY sex")->result();
    ...
    //Boxes and graphs end here--------------------------------
        ...
        $data['announcements'] = $this->db->select('*')->from('announcements')->where('active', 1)->order_by('id', 'desc')->get()->result();
    ...
    public function manual_clocking_new()
    {
        ...
        if ($permissions_level == "Outlet") {
            ...
            if (empty($this->input->get("branch")) || $this->input->get("branch") != $bid) {
                redirect("overview/manual_clocking_new?branch=$bid&month=" . date('m') . "&year=" . date('Y'));
                return;
            }
        }
        ...
        if (!empty($this->input->get("month")) && !empty($this->input->get("year"))) {
            ...
            $where_date = " AND MONTH(datetime) = " . $this->input->get("month") . " AND YEAR(datetime) = " . $this->input->get("year");
        } else {
            redirect("overview/manual_clocking_new?month=" . date('m') . "&year=" . date('Y'));
            return;
        }
...
//  die();
    ...
    public function employee_report($emp_id = 0)
    {
        ...
        // $data["employees_dropdown"] = $this->db->query("SELECT employees.id, special_id,first_name,last_name FROM employees INNER JOIN roles ON employees.role_id = roles.id WHERE employees.deleted_at IS NULL AND employee_status = 'active' AND roles.exclude_from_system = 'no' AND employees.company_id = $cid $where_branch_2 ORDER BY special_id")->result();
        ...
        $render_clockings_query_for_employee_month = render_clockings_query_for_employee_month($emp_id, $this->input->get("month"), $this->input->get("year"));
         ...
         $shift_days = $this->db->query("SELECT (SELECT MIN(id) FROM $render_clockings_query_for_employee_month as clockings where DATE(clockings.clock_in) = shift_days.date AND clockings.employee_id=$emp_id) as id,

            (SELECT reason FROM $render_clockings_query_for_employee_month as clockings where DATE(clockings.clock_in) = shift_days.date AND clockings.employee_id=$emp_id LIMIT 1) as reason,

            r.remark as remark,

            (SELECT GROUP_CONCAT(DISTINCT name SEPARATOR ', ') FROM $render_clockings_query_for_employee_month as clockings INNER JOIN shifts ON clockings.shift_id = shifts.id where DATE(clockings.clock_in) = shift_days.date AND clockings.employee_id=$emp_id LIMIT 1) as shifts, 

            shift_days.shift_id,shifts.name, date, employees,

        (SELECT MIN(clock_in) FROM $render_clockings_query_for_employee_month as clockings where DATE(clockings.clock_in) = shift_days.date AND clockings.employee_id=$emp_id) as clock_in,

        (SELECT MAX(clock_out) FROM $render_clockings_query_for_employee_month as clockings where DATE(clockings.clock_in) = shift_days.date  AND clockings.employee_id=$emp_id) as clock_out,

        (SELECT auto_clock_out FROM $render_clockings_query_for_employee_month as clockings where DATE(clockings.clock_in) = shift_days.date  AND clockings.employee_id=$emp_id ORDER BY id DESC LIMIT 1) as auto_clock_out,

        (SELECT IFNULL(SUM(TIME_FORMAT(TIMEDIFF(a.`clock_out`, a.`clock_in` ),'%H')), 0) AS hours FROM $render_clockings_query_for_employee_month as  a WHERE a.employee_id=$emp_id AND clock_out IS NOT NULL AND DATE(a.clock_in) = shift_days.date) as hours,

        (SELECT IFNULL(SUM(TIME_FORMAT(TIMEDIFF(a.`clock_out`, a.`clock_in` ),'%i')), 0) AS hours FROM $render_clockings_query_for_employee_month as  a WHERE a.employee_id=$emp_id AND clock_out IS NOT NULL AND DATE(a.clock_in) = shift_days.date) as minutes,

        TIME_FORMAT(TIMEDIFF((SELECT MAX(clock_out) FROM $render_clockings_query_for_employee_month as clockings where DATE(clockings.clock_in) = shift_days.date AND clockings.employee_id=$emp_id), (SELECT Min(clock_in) FROM clockings where DATE(clockings.clock_in) = shift_days.date AND clockings.employee_id=$emp_id)),'%H:%i') as total_time,
...
//die();
    ...
    public function attendance_sheet()
    {
        ...
        $shift_data_array = $this->db->select('s.id, name, overnight, date, date as shift_date, employees, half_day, grace_time as shift_grace_time, start_time as shift_start_time, end_time as shift_end_time, is_leave as shift_is_leave, is_paid, is_paid as shift_is_paid')->from('shift_days sd')->join('shifts s', 's.id = sd.shift_id')->where('date >=', $prev_day)->where('date <=', $last_day)->where('company_id', $cid)->get()->result();
        ...
        foreach ($result as &$row) {
            ...
            $result2 = get_current_employee_clockings($employees_min_clockings, $emp_id);
            // echo $this->db->last_query();die;

            $result2 = merge_result_with_shifts($result2, $shift_data_list);
            ...
            foreach ($result2 as &$row2) {
                ...
                if ($row2["clock_in"] == null) {
                    ...
                    if ($manual_late) {
                        ...
                        if ($manual_clock_in > beautify_time($row2["shift_grace_time"])) {
                            $status = "late";
    ...
    function getSettings()
    {
        ...
        if ($permssion_level === "Company") {
            ...
            if ($ot_settings) {
                ...
                $data['ot_weekly_hours'] = (double)$ot_settings->ot_weekly_hours;
            ...
            foreach ($late_in_round_settings as $key => $v) {
                $late_in_round_settings[$key]->start = (double)$v->start;
                $late_in_round_settings[$key]->end = (double)$v->end;
                $late_in_round_settings[$key]->round_to = (double)$v->round_to;
            }
    ...
    public function get_deduction_settings()
    {
        ...
        if ($permssion_level === "Company") {
            ...
            if ($deduction) {
                ...
                $data['lateness_time'] = (double)$deduction->void_lateness_time_if_less_than;
