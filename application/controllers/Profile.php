...
public function index($id){
    ...
    $emp->is_daily_waged = ($emp->is_daily_waged == 0) ? "No" : "Yes";
...
public function get_emp_data(){
    ...
    $transfers = $this->db->select("t.*")->from("transfers t")->where("employee_id", $id)->get()->result();

    $data["transfers"] = $this->db->select("t.*, b.name new_name, ob.name old_name")->from("transfers t")->join("branches b", "b.id = t.branch_id")->join("branches ob", "ob.id = t.old_branch_id")->where("employee_id", $id)->get()->result();
