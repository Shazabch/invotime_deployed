...
class Overview extends CI_Controller {
    ...
    $join_roles['on'] = 'employees.role_id = roles.id';
        ...
        if(isset($branch)){
           ...
           // $boxes[] = stats_box("Departments", "departments","COUNT(1)","3",array('company_id' => $cid));
            ...
            $q_male = $this->db->query("SELECT COUNT(id) AS count FROM employees WHERE deleted_at IS NULL AND sex = 'Male' AND company_id = $cid")->row();
            $q_female = $this->db->query("SELECT COUNT(id) AS count FROM employees WHERE deleted_at IS NULL AND sex = 'Female' AND company_id = $cid")->row();
            ...
            $attendance_percentage["box_count"] = "NA";
    ...
    public function manual_clocking_new(){
        ...
        $data["employees_dropdown"] = $this->db->query("SELECT employees.id, special_id,first_name,last_name FROM employees INNER JOIN roles ON employees.role_id = roles.id WHERE employees.deleted_at IS NULL AND roles.exclude_from_system = 'no' AND employees.company_id = $cid $branch_where_filter ORDER BY special_id")->result();
    ...
    public function branch_report(){
        ...
        foreach ($data["branches"] as $branch){
            ...
            $total_employees = $this->db->query("SELECT COUNT(id) as total_employees FROM employees WHERE employees.deleted_at IS NULL AND branch_id = " . $branch->id . " AND deleted_at is NULL")->row()->total_employees;
            ...
            if(!empty($all_employees)){
                ...
                $emps_to_remove = $this->db->query("SELECT GROUP_CONCAT(id) as employees_to_remove FROM employees WHERE employees.deleted_at IS NULL AND id NOT IN($all_employees) AND $where_filter branch_id = $branch_id")->row()->employees_to_remove;
...
//  die();
    ...
    public function employee_report($emp_id){
        ...
        $emp = $this->db->query("SELECT * FROM employees WHERE employees.deleted_at IS NULL AND id = $emp_id")->row();
        ...
        $render_clockings_query_for_employee_month = render_clockings_query_for_employee_month($emp_id,$this->input->get("month"));
...
//die();
    ...
    public function attendance_sheet(){
        ...
        if($permissions_level == "Outlet"){
            ...
            if($limit_access_to_department == "yes"){
                ...
                if($this->input->get("branch") != $bid || $this->input->get("dep") != $department_id){
                    redirect("overview/attendance_sheet?dep=$department_id&branch=$bid&month=".date('m'));
                    return;
                }
            }
            else{
            ...
            if($limit_access_to_department == "yes"){
                ...
                if($this->input->get("dep") != $department_id){
                    redirect("overview/attendance_sheet?dep=$department_id&month=".date('m'));
                    return;
                }
            }
       ...
       // $month_template[]
        ...
        $total_records = $this->db->query("SELECT COUNT(1) as total_records FROM employees INNER JOIN roles ON employees.role_id = roles.id WHERE employees.deleted_at IS NULL AND roles.exclude_from_system = 'no' AND $where_filter")->row()->total_records;
        $limit = 10;
        $total_pages = ceil($total_records / $limit);
        $page = 1;
        if(!empty($this->input->get("page"))){
        ...
        $result = $this->db->query("SELECT employees.id,special_id, first_name FROM employees INNER JOIN roles ON employees.role_id = roles.id WHERE employees.deleted_at IS NULL AND roles.exclude_from_system = 'no' AND  $where_filter ORDER BY first_name LIMIT $skip,$limit")->result_array();
        ...
        foreach($result as &$row){
            ...
            $clockings_table = render_clockings_query_for_employee_month($emp_id,$this->input->get("month"));

            $result2 = $this->db->query("SELECT (
                
        SELECT MIN(id) FROM $clockings_table as clockings where DATE(clockings.clock_in) = shift_days.date AND shift_days.shift_id=clockings.shift_id AND clockings.employee_id=$emp_id) as id,

        (SELECT reason FROM $clockings_table as clockings where DATE(clockings.clock_in) = shift_days.date AND shift_days.shift_id=clockings.shift_id AND clockings.employee_id=$emp_id LIMIT 1) as reason,

            shift_days.shift_id,shifts.name, date as shift_date, employees,

        (SELECT MIN(clock_in) FROM $clockings_table as clockings where DATE(clockings.clock_in) = shift_days.date AND shift_days.shift_id=clockings.shift_id AND clockings.employee_id=$emp_id) as clock_in,

        (SELECT MAX(clock_out) FROM $clockings_table as clockings where DATE(clockings.clock_in) = shift_days.date AND shift_days.shift_id=clockings.shift_id AND clockings.employee_id=$emp_id) as clock_out, 

        (SELECT IFNULL(SUM(TIME_FORMAT(TIMEDIFF(a.`clock_out`, a.`clock_in` ),'%H')), 0) AS hours FROM $clockings_table as a WHERE a.employee_id=$emp_id AND clock_out IS NOT NULL AND DATE(a.clock_in) = shift_days.date) as hours,
...
//die($this->db->last_query());
        ...
        $data["public_holidays"] = get_public_holidays();
    ...
    public function shifts_assignment(){
        ...
        if($permissions_level == "Outlet"){
            ...
            if($limit_access_to_department == "yes"){
                $where_department = " AND id = $department_id "; 
                if($this->input->get("branch") != $bid || $this->input->get("dep") != $department_id){
                    redirect("overview/shifts_assignment?dep=$department_id&branch=$bid&month=".date('m'));
                    return;
                }
            }
            else{
            ...
            if($limit_access_to_department == "yes"){
                $where_department = " AND id = $department_id "; 
                if($this->input->get("dep") != $department_id){
                    redirect("overview/shifts_assignment?dep=$department_id&month=".date('m'));
                    return;
                }
            }
    ...
    public function shifts_calendar(){
        ...
        if($permissions_level == "Outlet"){
            ...
            if($limit_access_to_department == "yes"){
                $where_department = " AND id = $department_id "; 
                if($this->input->get("branch") != $bid || $this->input->get("dep") != $department_id){
                    redirect("overview/shifts_calendar?dep=$department_id&branch=$bid&month=".date('m'));
                    return;
                }
            }
            else{
            ...
            if($limit_access_to_department == "yes"){
                $where_department = " AND id = $department_id "; 
                if($this->input->get("dep") != $department_id){
                    redirect("overview/shifts_calendar?dep=$department_id&month=".date('m'));
                    return;
                }
            }
