...
class Summary extends CI_Controller {
	...
	public function view($id){
		...
		$data['employee'] = $this->db->select('e.id as emp_id,first_name,special_id,d.name as department,p.title as position')->from('employees e')->join('departments d','d.id = e.department_id')->join('positions p','p.id = e.position_id')->where('e.id',$id)->get()->row();
		if(empty($_GET)){
		...
		foreach($period as $date) {
			...
			$date_string = $date->format('d M Y, D');
			$is_ot = false;
			$result = $this->db->select('c.id,"'.$date_string.'" as day_f, date_format(clock_in,"%H:%i") as clock_in, date_format(clock_in,"%d-%m-%Y %H:%i") as clock_in_1,date_format(clock_out,"%H:%i") as clock_out,date_format(clock_out,"%d-%m-%Y %H:%i") as clock_out_1,s.name,reason,date_format(overtime_starts,"%H:%i") as overtime_starts,is_ot,time_format(timediff(end_time,start_time),"%H:%i") as shift_hours, fixed_ot, fixed_overtime',false)->from('clockings c')->join('shifts s','c.shift_id = s.id','left')->join('ot_days od','od.employee_id = c.employee_id and od.ot_date = date(clock_in)','left')->where('date(clock_in)',$obj->date)->where('c.employee_id',$id)->get()->result();
			...
			if(!$obj->clockings){
				$shift_name = "";
				$shift = $this->db->select('name')->from('shift_days sd')->join('shifts s','s.id = sd.shift_id')->where('FIND_IN_SET('.$id.',employees)>',0)->where('date',$obj->date)->get()->row();
				if($shift){
