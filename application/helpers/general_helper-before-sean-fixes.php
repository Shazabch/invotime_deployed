...
function get_shift_list($id, $first_day, $last_day)
{
  ...
  $ci = &get_instance();
  $result = $ci->db->select(
    "s.id,round_off_ot,s.name,acting_code,code,shift_code,overnight,same_day_overnight,s.half_day,is_leave,is_paid,void_late_in,void_early_out,date,end_time, is_approved,timediff(end_time,grace_time) as full_time, timediff(timediff(end_time,grace_time) ,break_duration) as work_time, is_rest_day,
