 ...
 * @param string $summary_type  space separated summary
...
function calculate_summary_data_special($emp_id, $first_day, $last_day, $summary_type = "summary", $employee = false, $result_list = false, $result_list_overnight = false, $company_working_hours = false, $public_holidays = false, $ot_settings = false, $early_ot_settings = false, $approved_ot_list = false, $rest_and_off_days = false, $cid = null, &$worked_rest_days_array = [], &$worked_off_days_array = [], &$worked_holidays_array = [], &$unpaid_leaves_absent_days = [], $clockings_news = null, $clockings_news_overnight = null, &$paid_leaves_array = [], &$daily_ot_array = [], &$daily_late_array = [], $result_list_preshift = false, $clockings_news_preshift = null)
{
    ...
    foreach ($period as $date) {
        ...
        $obj->date_string = $date_string;
                // ============================================
        // EXCLUDE PH/SUNDAY FROM FULL_SPECIAL REPORT
        // ============================================
        $isExcludedDay = is_short_report_excluded_day($obj->date, $obj->day_name, $public_holidays);

        if ($isExcludedDay && $summary_type === "summary") {
            // For full_special report: show date but NO clocking data
            // Reset all clocking-related values
            $result = [];
            $clockings_news_result = [];
            $obj->first_in = "";
            $obj->last_out = "";
            $obj->first_in_o = "";
            $obj->last_out_o = "";
            ...
            $obj->is_ot = false;
            $obj->is_late = false;
            $obj->is_late_break = false;
            $obj->is_early_out = false;
            $obj->overtime_ph_x2 = "";
            $obj->overtime_ph_x3 = "";
            $obj->x2 = false;
            $obj->x3 = false;
            $obj->is_manual_exist = false;
            ...
            // Keep shift info for display
            $shift = search_from_list($shift_list, $obj->date);
            if ($shift) {
                $obj->shift_name = $shift->name;
                $obj->shift_code = $shift->code;
                $obj->acting_code = str_replace(",", "|", $shift->acting_code);
            }

            // Add remark for PH
            if (in_array($obj->date, $public_holidays)) {
                $ph = get_public_holiday_by_date($obj->date, $employee->branch_id, $cid);
                $obj->clockings[0] = new stdClass();
                $obj->clockings[0]->day_f = $date_string;
                $obj->clockings[0]->name = $ph && $ph->name ? $ph->name : "Public Holiday";
                $obj->clockings[0]->code = "";
                $obj->clockings[0]->clock_in = "";
                $obj->clockings[0]->clock_out = "";
                $obj->clockings[0]->total_time = "";
                $obj->clockings[0]->remark = "";
                $obj->clockings[0]->staff_remark = "";
                $obj->clockings[0]->is_break = false;
            } else if ($obj->day_name === 'Sunday') {
                $obj->clockings[0] = new stdClass();
                $obj->clockings[0]->day_f = $date_string;
                $obj->clockings[0]->name = "";
                $obj->clockings[0]->code = "";
                $obj->clockings[0]->clock_in = "";
                $obj->clockings[0]->clock_out = "";
                $obj->clockings[0]->total_time = "";
            ...
            $dates[] = $obj;
            continue; // Skip to next day - don't accumulate totals
        }
        // ============================================
        $obj->shift_hours = "";
        ...
        // ============================================
        // OT ACCUMULATION — EXCLUDE PH/SAT/SUN FOR SHORT REPORT
        // ============================================
         $isExcludedDay = is_short_report_excluded_day($obj->date, $obj->day_name, $public_holidays);

        if (!$isExcludedDay) {
            // Normal day OR full report — accumulate OT normally
            if ($obj->is_ot) {
                if (in_array($obj->date, $public_holidays) || $is_replaced_ph) {
                    if ($obj->x3) {
            ...
            // EXCLUDED DAY — don't accumulate any OT to monthly totals
            // (OT values already cleared above in the totals block)
        }
        ...
        // ============================================
        // EXCLUDE PH, SATURDAY, SUNDAY FROM SHORT REPORT TOTALS
        // ============================================
        ...
        if (!$isExcludedDay) {
            ...
            /////////
            // EXCLUDED DAY (PH, Sat, Sun) for Short Report
            // Do NOT add to totals — pass zeros to lateness so it doesn't count
            $late_result = get_lateness_time($total_late, "00:00", "00:00", "00:00", "00:00", false, false, false, false, $late_count);
            $total_late = $late_result[0];
            $late_count = $late_result[1];
            $today_late = $late_result[2];

            // Also clear OT values for short report display
            $obj->overtime = "";
            $obj->overtime_m = "";
            $obj->overtime_ph_x2 = "";
            $obj->overtime_ph_x3 = "";
...
/**
 * Check if a date should be excluded from short report totals
 * (Public Holiday, Saturday, or Sunday)
 */
function is_short_report_excluded_day($date, $day_name, $public_holidays)
{
