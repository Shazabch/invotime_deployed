...
//        die();
        ...
        $q = $this->db->select('employees.*,companies.organization_id, companies.lock_radius as is_lockradius, companies.clock_selfie as clock_selfie, companies.performance_data, companies.bluetooth_remark, companies.face_required, companies.toggle_camera_selfie, coordinate, branches.invalid_clocking_distance')->from('employees')
            ->join('companies', 'companies.id=employees.company_id')
...
//                $this->db->insert('users_authentication',array('users_id' => $id,'token' => $token,'expired_at' => $expired_at));
    ...
    {
        $q = $this->db->select('employees.*,companies.organization_id, companies.qr_code as is_qr, companies.include_in_qr_code, companies.exclude_from_qr_code, companies.self_clocking as is_self_clocking, include_in_self_clocking, exclude_from_self_clocking, companies.bluetooth as is_bluetooth, companies.lock_radius as is_lockradius, companies.clock_selfie, companies.performance_data, companies.bluetooth_remark, companies.clocking_remarks_required, companies.face_required, companies.toggle_camera_selfie, include_in_bluetooth, exclude_from_bluetooth, device_id, coordinate, branches.invalid_clocking_distance, devices.branch_id device_branch_id')->from('employees')
            ->join('companies', 'companies.id=employees.company_id')
            ->join('branches', 'employees.branch_id = branches.id')
            ->join('devices', 'employees.branch_id = devices.branch_id', 'left')
        ...
        $devices = $this->db->select("mac_address, coordinate, uuid, branch_id")->from("devices")->where("company_id", $q->company_id)->get()->result();
...
//                $this->db->insert('users_authentication',array('users_id' => $id,'token' => $token,'expired_at' => $expired_at));
    ...
    /**
     * Reset device id of a user
     * @param int $employeeId Auto increment id of the employee
     * @return array Response array
     */
    public function reset_device_id($employeeId)
    {
        $this->db->trans_start();
        $this->db
            ->where('id', $employeeId)
            ->update(
                'employees',
                array('user_device_id' => null, 'user_device_type' => null)
            );
        if ($this->db->trans_status() === false) {
            $this->db->trans_rollback();
            return array('status' => 500, 'message' => 'Internal server error.');
        ...
        $this->db->trans_commit();
        return array('status' => 200, 'message' => 'Successfully reset device id.');
    }
}
