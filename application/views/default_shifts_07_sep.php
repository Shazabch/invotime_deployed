...
<div class="page-wrapper" ng-app="myApp" ng-controller="shiftCtrl" ng-init="getBranchesAndOutlets()">
				...
				<div class="row">
					<div class="col-md-4">
						<div class="form-group">
							<label>Outlet</label>
							<select  class="form-control" ng-model="filterModel.branch" required="" ng-change="getShifts();filterEmployees();">
	...
	<script type="text/javascript">
		$(document).ready(function () {
			...
			$(".shift_days").select2();
			$(".public_holidays").select2({placeholder: "Exclude All"});
			$(".employees").select2({placeholder: "All Employees"});
			$(".departments").select2({placeholder: "All Departments"});
		...
		app.controller('shiftCtrl', function($scope,$http) {
			...
			$scope.getShifts = function(){
				$('body').LoadingOverlay("show",{maxSize:50});
				$http.post(base_url + 'default_shifts/getShifts', {branch_id: $scope.filterModel.branch}, config).then(function (response) {
			...
			$scope.filterEmployees = function(){
				$('body').LoadingOverlay("show",{maxSize:50});
				$http.post(base_url + 'default_shifts/getEmployees', {branch: $scope.filterModel.branch, department: $scope.filterModel.department}, config).then(function (response) {
			...
			$scope.assignShifts = function(){
				$('.submit-btn').LoadingOverlay("show",{maxSize:50});
			...
			$scope.getProgress = function(){
				...
				$http.post(base_url + 'default_shifts/getProgress', { session_id: $scope.filterModel.session_id }, config).then(function (response) {
