<div class="page-wrapper">
	...
	<div class="content container-fluid">
		<div class="row">
			<div class="col-xs-4">
				<h4 class="page-title">Early Out Sheet</h4>
			</div>
		</div>
...
<button title="Approve All Late" data-toggle="tooltip" class="btn btn-default btn-xs all_button" data-emp-id = "<?php echo $emp['id'];?>" data-month = "<?php echo $selected_month;?>" data-year = "<?php echo $selected_year;?>"><i style="font-size:15px" class="fa fa-check"></i></button>
									...
									<?php foreach ($emp['early_out_data'] as $ed): ?>
										<td class="text-center">
											<b class="<?php if(!$ed['is_early_out']){ echo "strike";} ?>"><?php echo $ed['early_out_time']; ?></b><br/>
											<div class="btn-group btn-group-xs" style="min-width: 45px">
												<button type="button" class="btn btn-success status_btn btn_check" <?php if($ed['is_early_out']){ echo "disabled";} ?> data-emp-id = "<?php echo $ed['id'];?>" data-early_out_date = "<?php echo $ed['day'];?>" data-is-early_out = "1">
													<span class="fa fa-check"></span>
												</button>
												<button type="button" class="btn btn-danger status_btn btn_close" <?php if(!$ed['is_early_out']){ echo "disabled";} ?> data-emp-id = "<?php echo $ed['id'];?>" data-early_out_date = "<?php echo $ed['day'];?>" data-is-early_out = "0">
...
<script type="text/javascript">
	...
	$(".status_btn").on("click", function(e) {
		...
		var day = $(this).attr('data-early_out_date');
		var is_early_out = $(this).attr('data-is-early_out');

		$.ajax({
			type: "POST",  
			url: "<?php echo base_url() ?>early_out_days/change_status",
			data: {'id' : emp_id, 'day' : day, 'is_early_out' : is_early_out},
	...
	$(".all_button").on("click", function(e) {
		...
		$.ajax({
			type: "POST",  
			url: "<?php echo base_url() ?>early_out_days/approve_all_early_out",
