                            ...
                            <div class="col-sm-12">
                                ...
                                <div class="panel panel-primary">
                                    <div class="panel-body">
                                      <h4 class="page-title"><?php echo $pageTitle ?> (Clocking)
                                        <?php if (is_page_permitted('view')) : ?>
                                          <a title="Employee Summary" data-toggle="tooltip" href="<?php echo site_url() ?>summary/view/<?php echo $emp->id ?>?<?php echo "from=01%2F" . $_REQUEST['month'] . "%2F".$_REQUEST['year'] ?>&<?php echo "to=". last_day_of_month($_REQUEST['month']) ."%2F" . $_REQUEST['month'] . "%2F".$_REQUEST['year'] ?>" class="btn btn-default"><i style="font-size:20px" class="fa fa-address-card"></i></a>
                                        <?php endif ?>
                                      </h4>
        ...
        <select class="form-control" id="sel1" name="year">
          ...
          <?php endif; ?>
                                                        ...
                                                        <td>
                                                            ...
                                                            <?php
                                                                // Preshift shifts can have a clock-in the night before (e.g. 23:50)
                                                                // for a shift starting just after midnight (e.g. 00:10). Plain
                                                                // "H:i" string comparison would wrongly read 23:50 as later than
                                                                // 00:10, so normalize both around the midnight boundary first.
                                                               $is_preshift_day = isset($day->shift_is_preshift)
                                                                    && in_array($day->shift_is_preshift, ["Yes", "yes", 1, "1", true], true);
                                                                if ($is_preshift_day) {
                                                                    $clock_in_cmp = preshift_normalized_minutes(beautify_time($day->clock_in));
                                                                    $shift_start_cmp = preshift_normalized_minutes(beautify_time($day->shift_start_time));
                                                                    $shift_grace_cmp = preshift_normalized_minutes(beautify_time($day->shift_grace_time));
                                                                } else {
                                                                    $clock_in_cmp = beautify_time($day->clock_in);
                                                                    $shift_start_cmp = beautify_time($day->shift_start_time);
                                                                    $shift_grace_cmp = beautify_time($day->shift_grace_time);
                                                                }
                                                            ?>

                                                            <?php if($day->shift_is_leave == "yes"): ?>

                                                            <?php elseif($clock_in_cmp < $shift_start_cmp): ?>

                                                                <span class="label label-success">Early</span>

                                                             <?php elseif($clock_in_cmp > $shift_grace_cmp): ?>
                                                        ...
                                                        <td>
                                                            <?php if($clock_in_cmp > $shift_grace_cmp): ?>
                                                        ...
                                                        <td>
                                                          <button style="font-size:11px;max-width:100px;overflow:hidden;white-space: normal;" id="btn-remark-<?php echo $selected_emp_id . '-' . $day->date ?>" data-remark="<?php echo $day->remark ?>" data-id="<?php echo $selected_emp_id ?>" data-date="<?php echo $day->date ?>" type="button" class="btn btn-default btn-xs" aria-label="Add remark" data-toggle="modal" data-target="#remark-modal">
                                                                  <!-- <span class="fa fa-comment" aria-hidden="true"></span> -->
