<div class="page-wrapper">
	...
	<div class="content container-fluid">
		<div class="row">
			<div class="col-xs-4">
				<h4 class="page-title">Late Sheet</h4>
			</div>
		</div>
...
<a title="Clocking Data" data-toggle="tooltip" class="btn btn-default btn-xs" href="<?php echo base_url() ?>overview/manual_clocking_new?month=<?php echo $from_url_month ?>&year=<?php echo $from_url_year ?>&emp=<?php echo $emp["id"] ?>"><i style="font-size:15px" class="fa fa-hourglass-half"></i></a>
 
<a title="Employee Report" data-toggle="tooltip" class="btn btn-default btn-xs" href="<?php echo base_url() ?>overview/employee_report/<?php echo $emp["id"] ?>?<?php echo "month=" . $from_url_month ?>&year=<?php echo $from_url_year ?>"><i style="font-size:15px" class="fa fa-clock"></i></a>
...
<button title="Approve All Late" data-toggle="tooltip" class="btn btn-default btn-xs all_button" data-emp-id = "<?php echo $emp['id'];?>" data-month = "<?php echo $selected_month;?>" data-year = "<?php echo $selected_year;?>"><i style="font-size:15px" class="fa fa-check"></i></button>
									...
									<?php foreach ($emp['late_data'] as $ed): ?>
										<td class="text-center">
											<b class="<?php if(!$ed['is_late']){ echo "strike";} ?>"><?php echo $ed['late_time']; ?></b><br/>
											<div class="btn-group btn-group-xs" style="min-width: 45px">
												<button type="button" class="btn btn-success status_btn btn_check" <?php if($ed['is_late']){ echo "disabled";} ?> data-emp-id = "<?php echo $ed['id'];?>" data-late_date = "<?php echo $ed['day'];?>" data-is-late = "1">
													<span class="fa fa-check"></span>
												</button>
												<button type="button" class="btn btn-danger status_btn btn_close" <?php if(!$ed['is_late']){ echo "disabled";} ?> data-emp-id = "<?php echo $ed['id'];?>" data-late_date = "<?php echo $ed['day'];?>" data-is-late = "0">
...
<script type="text/javascript">
	...
	$(".status_btn").on("click", function(e) {
		...
		var day = $(this).attr('data-late_date');
		var is_late = $(this).attr('data-is-late');

		$.ajax({
			type: "POST",  
			url: "<?php echo base_url() ?>late_days/change_status",
			data: {'id' : emp_id, 'day' : day, 'is_late' : is_late},
	...
	$(".all_button").on("click", function(e) {
		...
		$.ajax({
			type: "POST",  
			url: "<?php echo base_url() ?>late_days/approve_all_late",
