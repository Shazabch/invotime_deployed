<div class="page-wrapper" ng-app="myApp" ng-controller="otCtrl">
	<div class="content container-fluid" ng-cloak>
		<div class="row">
			<div class="col-xs-4">
				<h4 class="page-title">Late in round Settings</h4>
			</div>
		</div>
			...
			<div class="col-md-8">
				<form ng-submit="onLDRoundSubmit()">
					...
					<div class="m-t-20" ng-if="bid">
						<button type="button" class="btn btn-info" ng-click="addRLDSetting()"><span class="fa fa-plus"></span> Add Lateness Deduction Round Setting</button>
...
<script type="text/javascript">
	...
	app.controller('otCtrl', function($scope, $http) {
		...
		$scope.getOtSettings = function() {
			...
			$http.post('<?php echo base_url(); ?>' + 'overview/getSettings', '', config).then(function(response) {
		...
		$scope.outletChange = function() {
			...
			$http.post('<?php echo base_url() ?>' + 'overview/getOutletSettings', {
				...
				$scope.round_settings = response.data.lateness_deduction_round_settings;
		...
		$scope.onLDRoundSubmit = function() {
			...
			if (isOverlapping($scope.round_settings)) {
				showNotification("Error", "Lateness Deduction Round settings are invalid", 'error');
				return;
			}
			$("body").LoadingOverlay("show", {
				size: 50
			});
			$http.post('<?php echo base_url() ?>' + 'overview/updateLDRoundSettings', {
