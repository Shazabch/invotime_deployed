<div class="page-wrapper" ng-app="myApp" ng-controller="summaryCtrl">
	<style type="text/css">
		...
		.filter-card--modern {
			...
			box-shadow: 0 4px 24px rgba(30, 34, 90, 0.06), 0 1px 3px rgba(30, 34, 90, 0.04);
			overflow: hidden;
			margin-top: 20px;
		}
		...
		/* Inputs / selects (native fallback) */
		...
		.filter-card--modern .form-control:focus {
			border-color: #6366f1;
			background: #fff;
		...
		/* Select2 restyle */
		.filter-card--modern .select2-container--default .select2-selection--single {
			height: 42px;
			border: 1.5px solid #e7e9f2;
			border-radius: 10px;
			background: #f8f9fd;
		...
		.filter-card--modern .select2-container--default .select2-selection--single .select2-selection__rendered {
			line-height: 42px;
			color: #2c2e4a;
			font-weight: 500;
		}

		.filter-card--modern .select2-container--default .select2-selection--single .select2-selection__arrow {
			height: 40px;
		}

		.filter-card--modern .select2-container--default.select2-container--open .select2-selection--single,
		.filter-card--modern .select2-container--default .select2-selection--single:hover {
			border-color: #6366f1;
			box-shadow: 0 0 0 4px rgba(99, 102, 241, 0.12);
		}
		...
		/* Employee prev/next nav */
		.emp-filter-col--modern {
			display: flex;
			/* align-items: flex-center; */
			gap: 8px;
		}

		.emp-filter-col--modern .filter-field {
			flex: 1;
		}
		...
		.filter-card--modern .action-btn:hover,
		.filter-card--modern .action-btn:focus {
			transform: translateY(-2px);
		...
		.filter-card--modern .action-btn--pdf {
			background: linear-gradient(135deg, #f43f5e, #e11d48);
			box-shadow: 0 3px 10px rgba(225, 29, 72, 0.25);
		}

		.filter-card--modern .action-btn--excel {
			background: linear-gradient(135deg, #10b981, #059669);
			box-shadow: 0 3px 10px rgba(5, 150, 105, 0.25);
		}

		.filter-card--modern .action-btn--warn {
			background: linear-gradient(135deg, #f59e0b, #d97706);
			box-shadow: 0 3px 10px rgba(217, 119, 6, 0.25);
		}

		.filter-card--modern .action-btn--print {
		...
		/* ===== Date filter card (modern) ===== */
		.date-filter-form--modern {
			display: flex;
			flex-wrap: wrap;
		...
		.date-filter-form--modern .filter-field {
			flex: 1 1 160px;
			min-width: 140px;
			margin: 0;
		}

		.date-filter-form--modern .btn-primary {
			height: 42px;
			border-radius: 10px;
			padding: 0 22px;
			font-weight: 600;
		...
		.date-filter-form--modern .btn-primary:hover {
			transform: translateY(-2px);
			box-shadow: 0 8px 18px rgba(79, 70, 229, 0.32);
		}
		...
		.info-date-row>[class*="col-"] {
			display: flex;
			margin-bottom: 20px;
		}

		.info-date-row>[class*="col-"]>.card-box,
		.info-date-row>[class*="col-"]>.info-card--modern {
			width: 100%;
		}

		/* Pressed / active feedback */
		.filter-card--modern .action-btn:active,
		.nav-btn:active {
			transform: translateY(0);
			box-shadow: 0 2px 6px rgba(30, 34, 90, 0.18) !important;
			filter: brightness(0.96);
		}

		/* Accessible focus ring (keyboard nav) */
		.filter-card--modern .action-btn:focus-visible,
		.nav-btn:focus-visible,
		.filter-card--modern .form-control:focus-visible {
			outline: 2px solid #4f46e5;
			outline-offset: 2px;
		}

		/* Group divider between action clusters */
		...
		/* Export/print loading feedback */
		.filter-card--modern .action-btn.is-loading {
			pointer-events: none;
			opacity: 0.75;
		}

		.filter-card--modern .action-btn.is-loading .fa {
		...
		/* Icon-only compact mode on very small screens */
		@media (max-width: 380px) {
			...
			.filter-card--modern .action-btn .btn-label {
				position: absolute;
				width: 1px;
				height: 1px;
				overflow: hidden;
		...
		@media (max-width: 767px) {
			...
			.filter-card--modern [style*="width:300px"] {
				width: 100% !important;
			}
					...
					<?php if (in_array($company_id, [3])) : ?>
						...
						<div class="row">
							<div class="col-md-6">
								...
								<div class="pull-right">
									<input type="checkbox" ng-model="settings.is_employee_off_day" ng-change="update_employee_off_day()">
								</div>
							</div>
						</div>
					<?php endif; ?>
	...
	<div class="content container-fluid">
		<div class="row">
			<div class="col-md-12">
				<div class="card-box filter-card--modern hide-on-print">
					<div class="row">
						<div class="col-md-12">
							<div class="filter-card-header">
								<h3><i class="fa fa-id-badge"></i> Employee Summary</h3>
							</div>
						</div>
						<div class="col-md-4">
							<div style="width:300px" class="form-group filter-field">
								<label><i class="fa fa-sitemap"></i> Department</label>
						...
						<div class="col-md-4 col-lg-3 col-sm-6">
							...
							<div class="emp-filter-col emp-filter-col--modern">

								<button class="btn btn-default nav-btn nav-btn--prev" id="prevEmployee" type="button">
									<i class="fa fa-arrow-left"></i> Previous
								</button>
								<div class="form-group filter-field">

									<select class="form-control apply-select2 custom-resize-input" id="emp" name="emp">
								...
								<button class="btn btn-default nav-btn nav-btn--next" id="nextEmployee" type="button">
						...
						<div class="col-md-12">
							<div class="filter-actions hide-on-print">
								<button class="btn btn-primary action-btn action-btn--print" title="Print" onclick="window.print()"><i class="fa fa-print"></i> <span class="btn-label">Print</span></button>

								<a class="btn btn-primary action-btn action-btn--pdf" title="Export as PDF" target="_blank" href="<?php echo base_url() ?>summary/pdf/<?php echo $employee->emp_id ?>/<?php echo $from_p ?>/<?php echo $to_p ?>" onclick="this.classList.add('is-loading'); var el=this; setTimeout(function(){ el.classList.remove('is-loading'); }, 3000);"><i class="fa fa-file-pdf-o"></i> <span class="btn-label">Export as PDF</span></a>
								<a class="btn btn-primary action-btn action-btn--excel" title="Export as Excel" target="_blank" href="<?php echo base_url() ?>exports/excel/<?php echo $employee->emp_id ?>/<?php echo $from_p ?>/<?php echo $to_p ?>" onclick="this.classList.add('is-loading'); var el=this; setTimeout(function(){ el.classList.remove('is-loading'); }, 3000);"><i class="fa fa-file-excel-o"></i> <span class="btn-label">Export as Excel</span></a>

								<?php if (is_page_permitted('manual_clocking_new') || is_page_permitted('shifts_assignment')) : ?>
									<span class="action-group-divider"></span>
								<?php endif; ?>
								<?php if (is_page_permitted('manual_clocking_new')) : ?>
									<a class="btn btn-primary action-btn" title="Clocking Data" href="<?php echo base_url() ?>overview/manual_clocking_new?month=<?php echo date("m") ?>&emp=<?php echo $employee->emp_id ?>"><i class="fa fa-clock-o"></i> <span class="btn-label">Clocking Data</span></a>
								<?php endif ?>
								<?php if (is_page_permitted('shifts_assignment')) : ?>
									<a class="btn btn-primary action-btn" title="Shift Assignment" href="<?php echo base_url() ?>overview/shifts_assignment?emp=<?php echo $employee->emp_id ?>&month=<?php echo date('m', strtotime($to_p)) ?>&year=<?php echo date('Y', strtotime($to_p)) ?>"><i class="fa fa-calendar"></i> <span class="btn-label">Shift Assignment</span></a>
								<?php endif ?>

								<?php if ($is_alternate_clockings) : ?>
									<span class="action-group-divider"></span>
									<a class="btn btn-primary action-btn action-btn--warn" title="Fix Clockings" href="<?php echo base_url() ?>summary/fix_clockings/<?php echo $employee->emp_id ?>/?from=<?php echo urlencode($from_f) ?>&to=<?php echo urlencode($to_f) ?>"><i class="fa fa-wrench"></i> <span class="btn-label">Fix Clockings</span></a>
									<a class="btn btn-primary action-btn action-btn--warn" title="Reset Clockings" href="<?php echo base_url() ?>summary/reset_clockings/<?php echo $employee->emp_id ?>/?from=<?php echo urlencode($from_f) ?>&to=<?php echo urlencode($to_f) ?>"><i class="fa fa-refresh"></i> <span class="btn-label">Reset Clockings</span></a>
								<?php endif; ?>
								<span class="action-group-divider"></span>
								<a class="btn btn-primary action-btn" title="Refresh Shifts" href="<?php echo base_url() ?>summary/refresh_shifts/<?php echo $employee->emp_id ?>/?from=<?php echo urlencode($from_f) ?>&to=<?php echo urlencode($to_f) ?>"><i class="fa fa-refresh"></i> <span class="btn-label">Refresh Shifts</span></a>
				...
				<div class="row info-date-row">
					<div class="col-md-6">
						<div class="info-card--modern">
							<div class="info-card-title"><i class="fa fa-id-card-o"></i> Personal Info</div>
							<ul class="personal-info personal-info--grid">
								<li>
					...
					<div class="col-md-6 hide-on-print">
						<div class="info-card--modern">
							<div class="info-card-title"><i class="fa fa-calendar-o"></i> Date Range</div>
							<form class="date-filter-form--modern" method="get" action="<?php echo base_url(); ?>summary/view/<?php echo $emp_id; ?>/<?php echo $selected_department; ?>">
								<div class="form-group filter-field">
									<label class="control-label"><i class="fa fa-calendar"></i> From<span class="text-danger">*</span></label>
									<input class="form-control datetimepicker" type="text" id="from" required="" name="from" autocomplete="off">
								</div>
								<div class="form-group filter-field">
									<label class="control-label"><i class="fa fa-calendar"></i> To<span class="text-danger">*</span></label>
			...
			<style>
				...
				.summary-freeze-table table thead tr th {
					text-align: center;
				}
			</style>


			<div class="card-box summary-table-zoom">
				<div class="row">
					<div class="col-md-12">
						<div class="table-responsive freeze-table summary-freeze-table">
								...
								<tbody>
									<?php foreach ($dates as $d) { ?>
										<?php foreach ($d->clockings as $key => $clock) { ?>
											<tr>
												<?php if ($key == 0) { ?>
													<td rowspan="<?php echo count($d->clockings); ?>" class="text-center <?php if (in_array($d->date, $public_holidays) || $d->is_replaced_ph) {
																																echo 'holiday';
																															} ?>" style="vertical-align: middle"><b <?php if (in_array($d->date, $public_holidays)) {
																																										echo "data-toggle='tooltip' data-html='true' data-placement='top' data-original-title='" . $public_holidays_names[array_search($d->date, $public_holidays)] . "'";
																																									} ?>><?php echo $clock->day_f; ?></b><br>
												...
												<?php } ?>
												<td class="text-center <?php if ($key % 2 == 1) {
																			echo 'text-danger';
																		} ?>" style="vertical-align: middle"><?php if ($key % 2 != 1) {
																													echo $clock->name;
																												} else {
																													echo "Break";
																												} ?> </td>
												...
												<!-- <td class="text-center <?php if ($key % 2 == 1) {
																				echo 'text-danger';
																			} ?>" style="vertical-align: middle"><?php if ($clock->clock_out == "") {
																														echo "";
																													} else {
																														echo $clock->total_time;
																													} ?></td> -->
												<!-- hours end -->
												<?php if ($key == 0) { ?>
													...
													<td rowspan="<?php echo count($d->clockings); ?>" class="text-center" style="vertical-align: middle"><?php echo $d->employee_shift_hours; ?></td>
													<td rowspan="<?php echo count($d->clockings); ?>" class="text-center" style="vertical-align: middle"><?php if ($d->break_hours != "00:00") {
																																								echo $d->break_hours;
																																							} ?></td>

													<td rowspan="<?php echo count($d->clockings); ?>" class="text-center" style="vertical-align: middle">
														<span class="<?php if (!$d->is_late) {
																			echo 'strike';
																		} else {
																			echo 'countLate';
																		} ?>" style="display: none"><?php echo $d->late_hours; ?></span>
														<?php if ($d->clockings[0]->clock_in != "") { ?>

															<?php if ($is_emp_summary_editable === TRUE) : ?>
																<button
																	style="font-size:16px;max-width:100px;overflow:hidden;white-space: normal;"
																	class="btn btn-default btn-xs editLateButton <?php if (!$d->is_late) {
																														echo 'strike';
																													} ?>"
														...
														<?php } ?>
														<span class="show-on-print <?php if (!$d->is_late) {
																						echo 'strike';
																					} ?>" style="display: none;"><?php echo $d->late_hours; ?></span>
													</td>
													...
													<td rowspan="<?php echo count($d->clockings); ?>" class="text-center" style="vertical-align: middle">
														<span class="<?php if (!$d->is_late_break) {
																			echo 'strike';
																		} else {
																			echo 'countLateBreak';
																		} ?>" style="display: none"><?php echo $d->break_late_hours; ?></span>
														<?php if ($d->clockings[0]->clock_in != "") { ?>

															<?php if ($is_emp_summary_editable === TRUE) : ?>
																<button style="font-size:16px;max-width:100px;overflow:hidden;white-space: normal;" class="btn btn-default btn-xs editLateBreakButton <?php if (!$d->is_late_break) {
																																																			echo 'strike';
																																																		} ?>" data-toggle="modal" data-target="#editLateBreakHours" data-date="<?php echo $d->date; ?>" data-empid="<?php echo $employee->emp_id; ?>" data-latehours="<?php echo $d->break_late_hours; ?>"><?php if (!empty($d->break_late_hours) && $d->break_late_hours != "00:00") {
																																																																																																				echo $d->break_late_hours;
																																																																																																			} else { ?><i class="fa fa-plus"></i> <?php } ?></button>
															<?php else : ?>
																<?php if (!empty($d->break_late_hours) && $d->break_late_hours != "00:00") {
																	echo $d->break_late_hours;
																}  ?>
															<?php endif ?>

														<?php } ?>
														<span class="show-on-print <?php if (!$d->is_late_break) {
																						echo 'strike';
																					} ?>" style="display: none;"><?php echo $d->break_late_hours; ?></span>
													</td>
													<!-- early out start -->
													<td rowspan="<?php echo count($d->clockings); ?>" class="text-center" style="vertical-align: middle">
														<span class="<?php if (!$d->is_early_out) {
																			echo 'strike';
																		} else {
																			echo 'countEarlyOut';
																		} ?>" style="display: none"><?php echo $d->early_out; ?></span>
														<?php if ($d->clockings[0]->clock_in != "") { ?>

															<?php if ($is_emp_summary_editable === TRUE) : ?>
																<button style="font-size:16px;max-width:100px;overflow:hidden;white-space: normal;" class="btn btn-default btn-xs editEarlyOutButton <?php if (!$d->is_early_out) {
																																																			echo 'strike';
																																																		} ?>" data-toggle="modal" data-target="#editEarlyOutHours" data-date="<?php echo $d->date; ?>" data-empid="<?php echo $employee->emp_id; ?>" data-earlyhours="<?php echo $d->early_out; ?>"><?php if (!empty($d->early_out) && $d->early_out != "00:00") {
																																																																																																		echo $d->early_out;
																																																																																																	} else { ?><i class="fa fa-plus"></i> <?php } ?></button>
															<?php else : ?>
																<?php if (!empty($d->early_out) && $d->early_out != "00:00") {
																	echo $d->early_out;
																}  ?>
															<?php endif ?>

														<?php } ?>
														<span class="show-on-print <?php if (!$d->is_early_out) {
																						echo 'strike';
																					} ?>" style="display: none;"><?php echo $d->early_out; ?></span>
													</td>
													<!-- early out end -->
													...
													<?php if ($d->clockings[0]->clock_in != "") { ?>
														...
														<?php else : ?>
														<?php if (!empty($d->short_hours) && $d->short_hours != "00:00") {
																echo $d->short_hours;
															}  ?>
														<?php endif ?>
													...
													<td rowspan="<?php echo count($d->clockings); ?>" class="text-center" style="vertical-align: middle">
														<?php if (!in_array($d->date, $public_holidays) && !in_array($d->day_name, $rest_days) && !in_array($d->day_name, $off_days) && $d->is_shift == 'true' && !$d->is_replaced_ph && !$d->is_rest_day  && !$d->is_employee_off_day) : ?>
															...
															<?php endif ?>
														<?php endif ?>
														<!-- <?php if (!in_array($d->date, $public_holidays) && !in_array($d->day_name, $rest_days) && !in_array($d->day_name, $off_days) && $d->is_shift == 'true') { ?><span class="otspan <?php if ($d->is_ot) {
																																																													echo 'countOT';
																																																												} ?>"><?php echo ($d->is_ot) ? add_time_minus($d->overtime, $d->overtime_m) : $d->overtime; ?></span><?php } ?> -->
													</td>
													...
													<?php endif; ?>
													<?php if ($tsf_custom_summary): ?>
														<td rowspan="<?php echo count($d->clockings); ?>" class="text-center" style="vertical-align: middle"><?= map_address($d->location) ?></td>
														<td rowspan="<?php echo count($d->clockings); ?>" class="text-center" style="vertical-align: middle"><?= $d->distance ? round($d->distance) . 'm' : '' ?></td>
													<?php endif; ?>
													<td rowspan="<?php echo count($d->clockings); ?>" class="text-center" style="vertical-align: middle">
														<?php if ($is_emp_summary_editable === TRUE): ?>
															<button
													...
													<?php if ($tsf_custom_summary): ?>
														<td rowspan="<?php echo count($d->clockings); ?>" class="text-center" style="vertical-align: middle"><?= $d->meal_days ?></td>
													<?php endif; ?>
													<td rowspan="<?php echo count($d->clockings); ?>" class="text-center" style="vertical-align: middle">
														<?php if ($is_emp_summary_editable === TRUE): ?>
															<button
																style="font-size:11px; max-width:100px; overflow:hidden; white-space: normal;"
																id="btn-staff-remark-<?php echo $emp_id; ?>-<?php echo $d->date; ?>"
																data-staff-remark="<?php echo $clock->staff_remark; ?>"
																data-id="<?php echo $emp_id; ?>"
																data-date="<?php echo $d->date; ?>"
																data-type="staff"
																type="button"
																class="btn btn-default btn-xs"
																aria-label="Add staff remark"
																data-toggle="modal"
																data-target="#remark-modal">
																<?php if (!empty($clock->staff_remark)): ?>
																	<?php echo $clock->staff_remark; ?>
																<?php else: ?>
																	<span class="fa fa-plus" aria-hidden="true"></span>
																<?php endif; ?>
															</button>
									...
									<?php if ($total != "00:00" || $work != "00:00" || $break != "00:00") { ?>
										<tr>
											...
											<td class="text-center month-overtime <?php if ($lateness_time != $lateness_time_deducted) {
																						echo 'strike';
																					} ?>" colspan="<?= $custom_in_outs ? 2 : 2; ?>"><?php echo $month_overtime; ?></td>
											<?php if (!$tsf_custom_summary): ?>
											...
											<?php else: ?>
												<td colspan="<?= $custom_in_outs ? 2 : 3; ?>"></td>
											<?php endif; ?>
										</tr>
										<tr>
											<td colspan="<?= $custom_in_outs ? 7 : 5; ?>"></td>
											<td colspan="3" class="text-center"><b>Lateness Time</b></td>
											<td colspan="3" class="text-center <?php if ($lateness_time != $lateness_time_deducted) {
																					echo 'strike';
																				} ?>"><?php echo $lateness_time; ?></td>
											<td colspan="4" class="text-center">
												<b>Deduct from OT</b>
											</td>
											<td colspan="1" class="text-center">
												<div class="btn-group btn-group-xs" style="min-width: 45px">
													<button type="button" class="btn btn-success status_btn btn_check" <?php if ($employee->deduct_from_ot) {
																															echo "disabled";
																														} ?> data-emp-id="<?php echo $employee->emp_id; ?>" data-deduct="1">
														<span class="fa fa-check"></span>
													</button>
													<button type="button" class="btn btn-danger status_btn btn_close" <?php if (!$employee->deduct_from_ot) {
																															echo "disabled";
																														} ?> data-emp-id="<?php echo $employee->emp_id; ?>" data-deduct="0">
														<span class="fa fa-close"></span>
													</button>
												</div>
											</td>
											<td colspan="<?= $tsf_custom_summary ? 4 : 6; ?>"></td>
										</tr>
										<?php if ($lateness_time != $lateness_time_deducted) { ?>
											<tr>
												...
												<td colspan="4" class="text-center"><?php echo $month_overtime_deducted; ?></td>
	...
	<div id="remark-modal" class="modal fade" role="dialog">
		<div class="modal-dialog modal-sm">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<input type="hidden" id="remark-type">
		...
		$(document).ready(function() {
			...
			function navigateEmployee(direction) {
				...
				if (direction === 'prev') {
					...
					newIndex = currentIndex + 1;
					if (newIndex >= $options.length) return; // Already at last
				}

				var newVal = $options.eq(newIndex).val();
				var selectedDepartment = $(".apply-select3").children("option:selected").val();
				var curr_page = window.location.href;
				var res = curr_page.split("?");
				var params = "";
				...
				window.location = js_base_url + 'summary/view/' + newVal + '/' + selectedDepartment + '/' + params;
			}

			$("#prevEmployee").click(function() {
				navigateEmployee('prev');
			});
	...
	<script>
		...
		$(document).ready(function() {
						...
						success: function(result) {
							...
							if (result) {
								...
								try {
									...
									$.notify(
										"Error: invalid response from server while updating trips.", {
											position: "top center",
											className: 'error',
											style: 'bootstrap',
											gap: 20,
			...
			// Store and restore scroll position for summary table
			var sumTableContainer = $('.summary-table-zoom .freeze-table');
			var scrollKey = 'summary_table_scroll_top';

			// Restore scroll position on page load
			setTimeout(function() {
				var savedScroll = sessionStorage.getItem(scrollKey);
				if (savedScroll) {
					sumTableContainer.scrollTop(parseInt(savedScroll));
				}
			}, 100);

			// Store scroll position when scrolling
			sumTableContainer.on('scroll', function() {
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$('#remark-modal').on('show.bs.modal', function(event) {
				...
				var type = button.data('type') || 'remark'; // default normal remark
				var remark = (type === 'staff') ? button.data('staff-remark') : button.data('remark');

				$(this).find("#remark-id").val(id);
				$(this).find("#remark-date").val(date);
				$(this).find("#remark-type").val(type); // hidden input for type
			...
			$("#btn-remark-save").on("click", function(e) {
				...
				var type = $("#remark-type").val();

				var url = "<?php echo base_url() ?>overview/save_remark";
				if (type === "staff") {
					url = "<?php echo base_url() ?>overview/save_staff_remark"; // new endpoint
				}
				...
				$.ajax({
					...
					success: function(result) {
						...
						if (result) {
							...
							var btnId = (type === "staff") ? '#btn-staff-remark-' + id + '-' + date : '#btn-remark-' + id + '-' + date;
							$(btnId).text(remark).attr("data-" + type + "-remark", remark);
						}
					}
				});
			});
			...
			$("#btn-remark-delete").on("click", function() {
				...
				var id = $("#remark-id").val();
				var date = $("#remark-date").val();
				var type = $("#remark-type").val(); // check type: staff or normal

				let url = "<?php echo base_url() ?>overview/save_remark";
				let data = {
					'id': id,
					'remark': '',
					'date': date
				...
				if (type === "staff") {
					url = "<?php echo base_url() ?>overview/save_staff_remark";
					data = {
						'id': id,
						'remark': '',
						'date': date
				...
				$.ajax({
					type: "GET", // better to use POST for save/delete
					url: url,
					data: data,
					success: function(result) {
						...
						if (result) {
							$('#remark-modal').modal("hide");
							if (type === "staff") {
								$('#btn-staff-remark-' + id + '-' + date).html('<span class="fa fa-plus" aria-hidden="true"></span>');
								$('#btn-staff-remark-' + id + '-' + date).attr("data-remark", '');
							} else {
								$('#btn-remark-' + id + '-' + date).html('<span class="fa fa-plus" aria-hidden="true"></span>');
								$('#btn-remark-' + id + '-' + date).attr("data-remark", '');
							}

							console.log("Remark deleted (" + type + ")");
						}
					},
					error: function() {
						$("#btn-remark-delete").LoadingOverlay("hide");
	...
	<script>
		...
		app.controller('summaryCtrl', function($scope, $http) {
			...
			$scope.getClockings = function(id, date, overnight, cut_off_time) {
				...
				$scope.clockings = [];
				$('#inputboxClockings').LoadingOverlay("show", {
					maxSize: 50
				});
				$http.post(base_url + 'summary/getClockings', {
					id: id,
					date: date,
					overnight: overnight,
			...
			$scope.saveClocking = function() {
				...
				$http.post(base_url + 'summary/saveClocking', {
					employee_id: $scope.employee_id,
					date: $scope.clockingDate,
					time: time,
					type: $scope.clockingType,
					id: $scope.clockingId,
					overnight: $scope.overnight
				}, config).then(function(response) {
			...
			$scope.deleteClocking = function(id) {
				if (confirm("Are you sure you want to delete this clocking?")) {
					...
					$http.post(base_url + 'summary/deleteClocking', {
						id: id
					}, config).then(function(response) {
						if (response.data.success) {
							$('#inputboxClockings').LoadingOverlay("hide");
			...
			$scope.refresh_shift = function() {
				...
				$http.post(base_url + 'summary/refresh_shift', {
					employee_id: $scope.settings.employee_id,
					date: $scope.settings.date,
					shift: $scope.prev_shift
				}, config).then(function(response) {
			...
			$scope.update_employee_off_day = function() {
				$('#settingsBox').LoadingOverlay("show", {
					maxSize: 50
				});
				$http.post(base_url + 'summary/update_employee_off_day', {
					employee_id: $scope.settings.employee_id,
					date: $scope.settings.date,
					is_employee_off_day: $scope.settings.is_employee_off_day
				}, config).then(function(response) {
					$('#settingsBox').LoadingOverlay("hide");
					$.notify(
						response.data.msg, {
