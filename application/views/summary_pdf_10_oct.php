...
<body>
	...
	<table style="page-break-inside: avoid;">
		...
		<tbody>
			<?php $count = 1; $page = 1; $records = 34; foreach($dates as $date_key => $d){ ?>
				<?php foreach($d->clockings as $key => $clock){ $row_span = ceil(count($d->clockings) / 2); if($key%2 == 0){ $count++; ?>
					<tr>
						<?php if($key == 0){ ?>
							<td rowspan="<?php echo $row_span;?>" class="text-center date" style="vertical-align: middle"><b><?php echo $clock->day_f; ?></b></td>
						<?php } ?>
						...
						<?php if($key == 0){ ?>
							...
							<!-- simple OT -->
							<!-- <td rowspan="<?php echo $row_span;?>" class="text-center <?php if(!$d->is_ot){echo 'strike';} ?>" style="vertical-align: middle;"><?php if(!in_array($d->date,$public_holidays) && !in_array($d->day_name, $rest_days) && $d->is_shift == 'true' && !$d->is_replaced_ph) echo ($d->is_ot) ? add_time_minus($d->overtime, $d->overtime_m) : $d->overtime ;  ?></td> -->
							...
							<td rowspan="<?php echo $row_span;?>" class="text-center remark" style="vertical-align: middle"><?php echo $clock->remark; ?></td>
						<?php
						 }
			...
			<?php } ?>
					...
					<tr>
						...
						<td class="text-center <?php if($lateness_time != $lateness_time_deducted){ echo 'strike'; } ?>" colspan="2"><?php echo $month_overtime;?></td>
						<td class="text-center"><?php echo $month_overtime_ph;?></td>
