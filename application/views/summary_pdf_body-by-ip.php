<div class="<?= $merged ? '' : 'header-fixed'; ?> header">
	...
	<p style="margin-top: 0px; margin-bottom: 0px;"><b>Position</b>: <?php echo $employee->position; ?>, <b>Department</b>: <?php echo $employee->department; ?> | Generated at <b><?php echo date("d/m/Y H:i:s"); ?></b> by <b><?php echo get_user()["first_name"]; ?></b></p>
			...
			<?php foreach($d->clockings as $key => $clock){ $row_span = ceil(count($d->clockings) / 2); if($key%2 == 0){ if (!$custom_in_outs) {$count++;} ?>
				<tr>
					...
					<?php if($key == 0){ ?>
						...
						<?php endif; ?>
						<?php if ($tsf_custom_summary): ?>
							<td rowspan="<?php echo $row_span;?>" class="text-center location" style="vertical-align: middle; padding: 0; margin: 0;"><?= map_address($d->location); ?></td>
							<td rowspan="<?php echo $row_span;?>" class="text-center" style="vertical-align: middle"><?= $d->distance ? round($d->distance) . 'm' : '' ?></td>
						<?php endif; ?>
						<td rowspan="<?php echo $row_span; ?>" class="text-center remark" style="vertical-align: middle; padding: 0; margin: 0;"><?php if (!empty($clock->remark)) { echo $clock->remark; } elseif (in_array($d->date, $public_holidays)) { echo $public_holidays_names[array_search($d->date, $public_holidays)]; } else { echo ''; }?></td>
						<?php if ($tsf_custom_summary): ?>
							<td rowspan="<?php echo $row_span; ?>" class="text-center remark" style="vertical-align: middle"><?php echo $d->meal_days?></td>
						<?php endif; ?>
						<td rowspan="<?php echo $row_span; ?>" class="text-center remark" style="vertical-align: middle; padding: 0; margin: 0;"><?php echo $clock->staff_remark?></td>
					<?php
						}
						if (!$custom_in_outs){
							if(($key == 0 && (strlen($clock->remark) > 50 || strlen($clock->reason) > 50 || strlen(map_address($d->location)) > 50 || strlen($clock->staff_remark) > 40))){
							$count++;
							if($key == 0 && strlen($clock->remark) > 55 || strlen($clock->reason) > 55 || strlen(map_address($d->location)) > 55 || strlen($clock->staff_remark) > 70){
								$count++;
								if (strlen($clock->staff_remark) > 80) {
									$count++;
									if (strlen($clock->staff_remark) > 100) {
									$count++;
									if (strlen($clock->staff_remark) > 110) {
										$count++;
										}
									}
								}
