		...
		<?php $count = 1; $page = 1; $records = 34; foreach($dates as $date_key => $d){ ?>
			<?php foreach($d->clockings as $key => $clock){ $row_span = ceil(count($d->clockings) / 2); if($key%2 == 0){ $count++; ?>
				<tr>
					...
					<!-- <?php $clock_in_check = admin_add_edit_check_clock_in(isset($d->clockings[0]->clock_in_id) ? $d->clockings[0]->clock_in_id : null); ?>
					<?php if ($clock_in_check == 1) { ?>
						<td class="text-center <?php if($key%2 == 1){echo 'text-danger';}?>"><b><?php echo $clock->clock_in; ?></b></td>
					<?php }else{ ?>
						<td class="text-center <?php if($key%2 == 1){echo 'text-danger';}?>"><?php echo $clock->clock_in; ?></td>
					<?php } ?>

					<?php $clock_out_check = admin_add_edit_check_clock_out(isset($d->clockings[0]->clock_out_id) ? $d->clockings[0]->clock_out_id : null); ?>
					<?php if ($clock_out_check == 1) { ?>
						<td class="text-center <?php if($key%2 == 1){echo 'text-danger';}?>"><b><?php echo $clock->clock_out; ?></b></td>
					<?php }else{ ?>
