				...
				<tr>
					...
					<?php if($key == 0){ ?>
						...
						<?php if(!$custom_in_outs): ?>
							...
							<td rowspan="<?php echo $row_span;?>" class="text-center remark" style="vertical-align: middle"><?php echo $clock->reason; ?></td>
						<?php endif; ?>
					...
					<?php
						...
						if (!$custom_in_outs){
							if(($key == 0 && (strlen($clock->remark) > 30 || strlen($clock->reason) > 30))){
							$count++;
							if(strlen($clock->remark) > 50 || strlen($clock->reason) > 50 ){
								$count++;
							}
							}
						}
