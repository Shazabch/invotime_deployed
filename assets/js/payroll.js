...
app.controller('payrollCtrl', function($scope,$http) {
	...
	$scope.getData = function(employee_id,process_id){
		...
		$http.post(base_url + 'payroll/getdata', '', config).then(function (response) {
			...
			$scope.epf_settings = response.data.epf_settings;
	...
	$scope.calculate = function(db = false, change_overtimes = true, change_eligible = false){
		...
		$scope.getEPF = function(b, type){
			if (b > 20000) {
				...
				let settings = $scope.epf_settings.filter(obj => {
					return obj.type === type;
				})
				if(settings.length != 0){
					let employee_share = b * settings[0].employee_rate / 100;
					let employer_share = 0;
					if (settings[0].employer_share_fix == "Y") {
						employer_share = settings[0].employer_share;
					} else {
						employer_share = b * settings[0].employer_rate / 100;
					}
					// round employee share and employer share to next integer
					employee_share = Math.ceil(employee_share);
					employer_share = Math.ceil(employer_share);
...
app.controller('customizedreportCtrl', function($scope,$http) {
	$scope.summary_types = [];
	$scope.summary_columns = [];

	$scope.getData = function(process_id){
		$('body').LoadingOverlay("show",{maxSize:50});
		$http.post(base_url + 'payroll/getdataCustomizedReport', '', config).then(function (response) {
			$scope.summary_types = response.data.payroll_summary_types;
	...
	$scope.loadConfiguration = function(){
		$('body').LoadingOverlay("show",{maxSize:50});
		$http.post(base_url + 'payroll/getSummaryColumns', {summary_type: $scope.summary_type}, config).then(function (response) {
			$scope.summary_columns = response.data.summary_columns;
	...
	$scope.saveSummaryColumns = function(){
		$('body').LoadingOverlay("show",{maxSize:50});
		if ($scope.summary_columns.length === 0) {
            alert('No data to save!');
            return;
        }
		$http.post(base_url + 'payroll/saveSummaryColumns', {summary_columns: $scope.summary_columns}, config).then(function (response) {
