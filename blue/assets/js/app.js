...
$(document).ready(function() {
	...
	var app_base_url = js_base_url;
	

	if($('.xcrud').length == 0 ){
      $.getScript( app_base_url+"assets/js/bootstrap-datetimepicker.min.js", function( data, textStatus, jqxhr ) {
	...
	$(".nested").find(".active").closest(".submenu").find(".main-menu").addClass('subdrop');
	//$(".nested").find(".active").closest(".submenu").addClass('active');
	$(".nested").find(".active").closest("ul").css("display","block");
	...
	//$('#datatable_emp thead tr').clone(true).appendTo( '#datatable_emp thead' );

	if ($('#datatable_emp').length > 0) {
		// Clone the header row and append it
		$('#datatable_emp thead tr').clone(true).appendTo('#datatable_emp thead');
		$('#datatable_emp thead tr:eq(1) th').each(function (i) {
			var title = $(this).text();
			if (title === "Action" || title === "Photo") {
				$(this).html("");
				return;
			}
	
			var searchValue = localStorage.getItem('search_col_' + i) || '';
			$(this).html('<input class="form-control" style="max-width:145px;" type="text" placeholder="Search ' + title + '" value="' + searchValue + '" />');
			...
			$('input', this).on('keyup change', function () {
				...
				localStorage.setItem('search_col_' + i, value);
			});
		});
	
		var table = $('#datatable_emp').DataTable({
			orderCellsTop: true,
			fixedHeader: true,
			order: []
		});
	
		// Apply stored search terms
		table.columns().every(function (index) {
			var searchValue = localStorage.getItem('search_col_' + index);
			if (searchValue) {
				this.search(searchValue).draw();
			}
		});
	
		// Clear localStorage after 2 minutes (120000 milliseconds)
		setTimeout(function () {
			console.log('Clearing stored search terms...');
			for (var i = 0; i < table.columns().count(); i++) {
				localStorage.removeItem('search_col_' + i);
			}
