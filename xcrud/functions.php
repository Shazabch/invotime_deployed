...
function makeClockingTimeUpdate($postdata, $primary, $xcrud)
{
    ...
    if ($checking['old_clocking_type'] != $checking['$new_clocking_type'] 
        ...
        $db->query("INSERT INTO edited_clockings (clocking_id, employee_id, `type`, `datetime`)
            VALUES ('$primary', '$employee_id', '" . $old_clocking['type'] . "', '" . $old_clocking['datetime'] . "')
...
function replace_remove_clocking($primary_key, $xcrud)
{
    ...
    $edited_clocking = $ci->db->get_where("edited_clockings", ["clocking_id" => $primary_key])->row();
    if($edited_clocking) {
        $ci->db->where("clocking_id", $primary_key)->update("edited_clockings", ["deleted_at" => date("Y-m-d H:i:s")]);
    }
...
function after_holiday_insertion($postdata, $primary_key)
{
    ...
    if ($postdata->get('branch_id') == 0) {
        $log_data['to_branch_id'] = 0;
        $log_data['to_outlet'] = 'Company';
    } else {
        $branch_name = $ci->db->select('name')->from('branches')->where('id', $postdata->get('branch_id'))->get()->row()->name;
        $log_data['to_branch_id'] = $postdata->get('branch_id');
...
{
    $ci = &get_instance();
    $public_holiday = $ci->db->select('ph.branch_id id, b.name, ph.holiday_date, ph.title')->from('public_holidays ph')->join('branches b', 'b.id = ph.branch_id', 'left')->where('ph.id', $primary_key)->get()->row();
...
function before_work_hour_insertion($postdata, $xcrud)
{
    $cid = get_user()['company_id'];
    $postdata->set("company_id", $cid);
}

function get_total_active_staffs($value, $fieldname, $primary, $row, $xcrud)
{
    $ci = &get_instance();
    $cid = $value;
    $ci->db->select('companies.package, companies.additional_staff, packages.max_outlets, packages.max_active_staff');
    $ci->db->join('packages', 'packages.id = companies.package', 'left');
    $ci->db->where('companies.id', $cid);
    $company_details = $ci->db->get('companies')->row();
    $company_max_active_staff = $company_details->max_active_staff + $company_details->additional_staff;
    $ci->db->join('roles', 'roles.id = employees.role_id', 'left');
    $ci->db->where('employees.company_id', $cid);
    $ci->db->where('employees.employee_status', 'active');
    $ci->db->where('roles.exclude_from_system', 'no');
    $ci->db->where('employees.deleted_at is null');
    $employees_of_company = $ci->db->get('employees')->result();
    $employees_of_company = count($employees_of_company);
    return $employees_of_company . ' / ' . $company_max_active_staff;
}

function get_total_outlets($value, $fieldname, $primary, $row, $xcrud)
{
    $ci = &get_instance();
    $cid = $value;
    $ci->db->select('packages.max_outlets');
    $ci->db->join('packages', 'packages.id = companies.package', 'left');
    $ci->db->where('companies.id', $cid);
    $company_details = $ci->db->get('companies')->row();
    $company_max_outlets = $company_details->max_outlets;
    $ci->db->where('company_id', $cid);
    $ci->db->where('deleted_at is null');
    $company_branches = $ci->db->get('branches')->result();
    $company_outlets = count($company_branches);
    return $company_outlets . ' / ' . $company_max_outlets;
}

function get_replacement_ph_status($value, $fieldname, $primary_key, $row, $xcrud)
{
    return $value ? '<i class="fa fa-check text-success" />' : '<i class="fa fa-times text-danger" />';
}