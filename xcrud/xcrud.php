...
// error_reporting(0); // error reporting (debug)
...
class Xcrud
{
    ...
    public static function get_requested_instance()
    {
        ...
        if (isset($_POST['xcrud']['instance']) && isset($_POST['xcrud']['key']) && isset($_POST['xcrud']['task'])) {
            ...
            isset($_GET['xcrud']['instance']) && isset($_GET['xcrud']['key']) && isset($_GET['xcrud']['task']) && $_GET['xcrud']['task'] ==
            'file'
        ) {
        ...
        if (
            isset($_SESSION['lists']['xcrud_session'][$inst_name]['key']) && $_SESSION['lists']['xcrud_session'][$inst_name]['key'] ==
            $key
        ) {
    ...
    public function mass_alert_create(
        $email_table = '',
        $email_column = '',
        $emeil_where = '',
        $subject = '',
    ...
    public function mass_alert_edit(
        $email_table = '',
        $email_column = '',
        $emeil_where = '',
        $subject = '',
    ...
    protected function render_custom_csv()
    {
        ...
        while ($row = $db->result->fetch_assoc()) // low level result process, saves memory
        {
            ...
            foreach ($this->columns as $field => $fitem) {
                $out[] = htmlspecialchars_decode(
                    strip_tags($this->_render_export_item($field, $row[$field], $row['primary_key'], $row)),
                    ENT_QUOTES
                );
            }
    ...
    protected function _create($postdata = array())
    {
        ...
        if ($this->condition) {
            foreach ($this->condition as $params) {
                ...
                if (array_key_exists($params['field'], $this->result_row) && $this->_compare(
                    $this->result_row[$params['field']],
                    $params['operator'],
                    $params['value']
                )) {
    ...
    protected function _entry($mode = 'edit', $postdata = array())
    {
        ...
        if ($this->condition) {
            foreach ($this->condition as $params) {
                ...
                $params['value'] = $this->replace_text_variables($params['value'], $this->result_row);
                if (isset($this->result_row[$params['field']]) && $this->_compare(
                    $this->result_row[$params['field']],
                    $params['operator'],
                    $params['value']
                )) {
    ...
    protected function _update($postdata, $primary)
    {
        ...
        foreach ($postdata as $key => $val) {
            if (isset($fields[$key]) && !isset($this->locked_fields[$key]) && !isset($this->custom_fields[$key])) {
                ...
                $this->field_null[$key], isset($this->bit_field[$key])));
                */
                $set[] = '`' . $fields[$key]['table'] . '`.`' . $fields[$key]['field'] . '` = ' . $this->prepare_query_field(
                    $val,
                    $key,
                    'edit'
                );
    ...
    protected function _remove()
    {
        ...
        if ($this->replace_remove) {
            ...
            if (in_array('image', $this->field_type) or in_array('file', $this->field_type) or in_array('fk_relation', $this->field_type)) // images && fk
            {
            ...
            foreach ($this->where as $key => $params) {
                ...
                if (!isset($params['custom'])) {
                    ...
                    if (is_array($params['value'])) {
                        ...
                        if (isset($this->subselect[$fieldkey])) {
                            $where_arr[] = $this->subselect_where($fieldkey) . $this->_cond_from_where($params['field']) . $db->escape(
                                $params['value'],
                            ...
                            $where_arr[] = $this->_where_field($params) . $this->_cond_from_where($params['field']) . $db->escape(
                                $params['value'],
                                isset($this->no_quotes[$fieldkey])
                            );
                        }
        ...
        if ($this->search && ($this->task == 'list' or $this->task == 'print' or $this->task == 'csv' or $this->after == 'list')) {
            ...
            if ($this->column && isset($search_columns[$this->column])) {
                ...
                elseif (isset($this->subselect[$this->column])) {
                    $where_arr[] = '(' . $this->subselect_query[$this->column] . ') LIKE ' . $db->escape_like($this->phrase, $this->search_pattern);
                    ...
                    switch ($this->field_type[$this->column]) {
                        ...
                        case 'time':
                            ...
                            if ($this->phrase['from'] && $this->phrase['to']) {
                                $where_arr[] = '(`' . $fitem['table'] . '`.`' . $fitem['field'] . '` BETWEEN ' . $db->escape(gmdate($format, (int)$this->phrase['from'])) . ' AND ' . $db->escape(gmdate($format, (int)$this->phrase['to'])) . ')';
                            } elseif ($this->phrase['from']) {
                                $where_arr[] = '(`' . $fitem['table'] . '`.`' . $fitem['field'] . '` >= ' . $db->escape(gmdate($format, (int)$this->phrase['from'])) . ')';
                            } elseif ($this->phrase['to']) {
                                $where_arr[] = '(`' . $fitem['table'] . '`.`' . $fitem['field'] . '` <= ' . $db->escape(gmdate($format, (int)$this->phrase['to'])) . ')';
                            }
                            break;
                        case 'select':
                        case 'radio':
                        ...
                        default:
                            if (isset($this->point_field[$key])) {
                                ...
                                $where_arr[] = 'CAST(`' . $fitem['table'] . '`.`' . $fitem['field'] . '` AS UNSIGNED) LIKE ' . $db->escape_like($this->phrase, $this->search_pattern);
                            } else {
                                $where_arr[] = '(`' . $fitem['table'] . '`.`' . $fitem['field'] . '` LIKE ' . $db->escape_like($this->phrase, $this->search_pattern) . ')';
                            }
                            break;
                    }
                }
                ...
                foreach ($search_columns as $key => $fitem) {
                    if (isset($this->relation[$key])) {
                        ...
                        $this->field_type[$key] == 'date' || $this->field_type[$key] == 'datetime' || $this->field_type[$key] ==
                        'timestamp' || $this->field_type[$key] == 'time'
                    ) {
                        if (preg_match('/^[0-9\-\:\s]+$/', $this->phrase)) {
                            $or_array[] = '`' . $fitem['table'] . '`.`' . $fitem['field'] . '` LIKE ' . $db->escape_like($this->phrase, $this->search_pattern);
                        }
                    } elseif (isset($this->point_field[$key])) {
                        $or_array[] = 'CONCAT(X(`' . $fitem['table'] . '`.`' . $fitem['field'] . '`),\',\',Y(`' . $fitem['table'] . '`.`' . $fitem['field'] .
                            '`)) LIKE ' . $db->escape_like($this->phrase, $this->search_pattern);
                    } elseif (isset($this->bit_field[$key])) {
                        $or_array[] = 'CAST(`' . $fitem['table'] . '`.`' . $fitem['field'] . '` AS UNSIGNED) LIKE ' . $db->escape_like($this->phrase, $this->search_pattern);
                    } else {
    ...
    protected function _build_order_by()
    {
        if (count($this->order_by)) {
            ...
            foreach ($this->order_by as $field => $direction) {
                ...
                } elseif (isset($this->subselect[$field]) or isset($this->columns[$field]) or isset($this->no_select[$field]) or !strpos(
                    $field,
                    '.'
            ...
            foreach ($this->inner_table_instance as $inst_name => $field) {
                if (isset($this->result_row[$field])) {
                    ...
                    $this->nested_rendered[$inst_name] =
                        '<div class="xcrud-nested-container xcrud-container"><div class="xcrud-ajax" id="xcrud-ajax-' . base_convert(
                            rand(),
                            10,
                            36
                        ) . '">' . $instance->render('list') . '</div></div>';
                }
            }
        }
    ...
    protected function create_bool($name, $value = '', $tag = array())
    {
        ...
        return $this->open_tag('div', $this->theme_config('checkbox_container')) . $this->open_tag('label') . $this->single_tag(
            $tag,
            $this->theme_config('bool_field'),
            $this->field_attr[$name],
            true
        ) . $this->close_tag('label') . $this->close_tag('div');
    }
    ...
    protected function create_hidden($name, $value = '', $tag = array())
    {
        return $this->single_tag($tag + array(
            ...
            'value' => $value,
            'name' => $name
        ), 'xcrud-input', $this->field_attr[$name], true);
    }
    ...
    protected function create_relation($name, $value = '', $tag = array(), $dependval = false)
    {
        ...
        if ($this->relation[$name]['depend_on'] && $dependval === false) {
            ...
            $db->query('SELECT `' . $this->relation[$name]['rel_field'] . '` AS `field`,' . $name_select . $this->get_relation_tree_fields($this->relation[$name]) . ' FROM `' . $this->relation[$name]['rel_tbl'] . '` ' . $where .
        ...
        if ($value) {
            ...
            $attr = array('src' => isset($this->upload_config[$name]['url']) ? $this->real_file_link($value, $this->upload_config[$name], true) :
                $this->file_link(
                    $name,
                    $this->primary_val,
                    (isset($this->upload_config[$name]['detail_thumb']) ? $this->upload_config[$name]['detail_thumb'] : false),
                    false,
                    $value
                ), 'alt' => '');
    ...
    protected function filter_image($filename, $field)
    {
        ...
        $watermark = (isset($settings['watermark']) && $settings['watermark']) ? $this->check_file(
            $settings['watermark'],
            'try_crop_image'
        ) : false;
    ...
    protected function _image_resize(
        $src_file,
        $dest_file,
        $new_size_w = false,
        $new_size_h = false,
        ...
        switch ($type) {
            case 1:
                ...
                if ($transparent_source_index !== -1) {
                    ...
                    $transparent_destination_index = imagecolorallocate(
                        $dstHandle,
    ...
    array(50, 50))
    {
        ...
        if ($srcHeight < $srcWidth) {
            $ratio = (float)($srcHeight / $new_size_h);
            $cpyWidth = round($new_size_w * $ratio);
            if ($cpyWidth > $srcWidth) {
                $ratio = (float)($srcWidth / $new_size_w);
                $cpyWidth = $srcWidth;
    ...
    protected function _custom_image_crop(
        $src_file,
        $dest_file,
        $new_size_w,
        $new_size_h,
    ...
    protected function _render_list_item($field, $value, $primary_val, $row)
    {
        ...
        if (isset($this->field_type[$field])) {
            switch ($this->field_type[$field]) {
                ...
                case 'file':
                    if ($value) {
                        $out .= $this->open_tag('a', '', array('target' => '_blank', 'href' => isset($this->upload_config[$field]['url']) ? $this->real_file_link($value, $this->upload_config[$field]) : $this->file_link($field, $primary_val)));
                ...
                case 'image':
                    if ($value) {
                        if (Xcrud_config::$images_in_grid) {
                            ...
                            $out .= $this->open_tag('a', '', array('target' => '_blank', 'href' => isset($this->upload_config[$field]['url']) ? $this->real_file_link($value, $this->upload_config[$field]) : $this->file_link($field, $primary_val, false, false, $value)));
    ...
    protected function make_mailto($txt)
    {
        if ($this->emails_label)
            return preg_replace(
                ...
                '<a target="_blank" href="mailto:$1@$2.$3">' . $this->emails_label['text'] . '</a>',
                $txt
            );
        else
            return preg_replace(
                '/([A-Za-z0-9_\-\.]+)\@([A-Za-z0-9_\-\.]+)\.([A-Za-z]{2,4})/',
                '<a target="_blank" href="mailto:$1@$2.$3">$1@$2.$3</a>',
                $txt
            );
    }
    ...
    protected function _render_list_buttons(&$row)
    {
        ...
        if (!isset($this->hide_button['duplicate']) && !$this->table_ro && $this->is_duplicate($row)) {
            $tag = array(
                'tag' => 'a',
                'class' => 'xcrud-action waves-effect',
    ...
    public static function check_url($url, $scr_url = false)
    {
        ...
        $doc_root = trim(
            str_replace('\\', '/', str_replace(array('/public_html', '/private_html'), '', $_SERVER['DOCUMENT_ROOT'])),
            '/'
        );
    ...
    protected function lang($text = '')
    {
        ...
        return htmlspecialchars((isset(self::$lang_arr[$langtext]) ? self::$lang_arr[$langtext] : $text),
            ENT_QUOTES,
    ...
    protected function theme_config($text = '')
    {
        ...
        return htmlspecialchars((isset($this->theme_config[$text]) ? $this->theme_config[$text] : ''), ENT_QUOTES, Xcrud_config::$mbencoding);
    }
        ...
        if ($buttons) {
            ...
            foreach ($this->limit_list as $limts) {
                ...
                $out .= $this->open_tag(array(
                    'tag' => 'button',
                    'type' => 'button',
                    'class' => $this->theme_config('limit_list') . $active . " waves-effect ",
                    'data-limit' => $limts
                ), 'xcrud-action') . $this->lang($limts) . $this->close_tag(array('tag' => 'button'));
            }
        ...
        foreach ($this->fields_output as $key => $item) {
            ...
            if (isset($this->fields[$item['name']]['tab']) && $this->fields[$item['name']]['tab']) {
                ...
                $raw_out[] = $this->open_tag($row, $row_class) . $this->open_tag($label, $this->theme_config('details_label_cell')) . $item['label'] .
                    $this->get_field_tooltip($item['name'], $mode) . $this->close_tag($label) . $this->open_tag($field, $this->theme_config('details_field_cell')) . $item['field'] . $this->close_tag($field) . $this->close_tag($row);
            }
        }
        if (isset($this->field_tabs[$mode]) or $this->default_tab !== false) {
            $tabs_header = $this->open_tag($tabs_block, $this->theme_config('tabs_container'), array('class' => 'xcrud-tabs')) . $this->open_tag($tabs_head, $this->theme_config('tabs_header_row'));
            ...
            if ($this->default_tab !== false && $raw_out) {
                ...
                $tabs_header .= $this->open_tag($tabs_row, $this->theme_config('tabs_header_cell') . ($k == 0 ? ' ' . $this->theme_config('tabs_first_element') : '')) . $this->open_tag($tabs_link, $this->theme_config('tabs_header_link') . ($k ==
                    0 ? ' ' . $this->theme_config('tabs_first_element') : ''), array('href' => '#' . $tabid)) . $this->default_tab . $this->close_tag($tabs_link) . $this->close_tag($tabs_row);
                $tabs_body .= $this->open_tag($tabs_pane, $this->theme_config('tabs_content_pane') . ($k == 0 ? ' ' . $this->theme_config('tabs_first_element') : ''), array('id' => $tabid)) . $this->open_tag($container, $this->theme_config('details_container')) .
            ...
            if ($this->nested_rendered && Xcrud_config::$nested_in_tab) {
                foreach ($this->nested_rendered as $tabname => $content) {
                    ...
                    $tabs_header .= $this->open_tag($tabs_row, $this->theme_config('tabs_header_cell') . ($k == 0 ? ' ' . $this->theme_config('tabs_first_element') : '')) . $this->open_tag($tabs_link, $this->theme_config('tabs_header_link') . ($k ==
                        0 ? ' ' . $this->theme_config('tabs_first_element') : ''), array('href' => '#' . $tabid)) . $tabname . $this->close_tag($tabs_link) .
                        $this->close_tag($tabs_row);
                    $tabs_body .= $this->open_tag($tabs_pane, $this->theme_config('tabs_content_pane') . ($k == 0 ? ' ' . $this->theme_config('tabs_first_element') : ''), array('id' => $tabid)) . $content . $this->close_tag($tabs_pane);
                    ++$k;
    ...
    protected function get_table_tooltip()
    {
        ...
        if ($this->table_tooltip) {
            ...
            $out .= $this->open_tag(array('tag' => 'i', 'class' => ($this->table_tooltip['icon'] ? $this->table_tooltip['icon'] : $this->theme_config('tooltip_icon'))));
        ...
        if ($this->is_search) {
            if (is_array($this->phrase) && isset($this->field_type[$this->column])) {
                switch ($this->field_type[$this->column]) {
                    case 'timestamp':
                    case 'datetime':
                        $phrase = array('from' => $this->unix2datetime((int)$this->phrase['from'], true), 'to' => $this->unix2datetime((int)$this->phrase['to'], true));
                        $is_daterange = true;
                        break;
                    case 'date':
                        $phrase = array('from' => $this->unix2date((int)$this->phrase['from'], true), 'to' => $this->unix2date((int)$this->phrase['to'], true));
                        $is_daterange = true;
                        break;
                    case 'time':
                        $phrase = array('from' => $this->unix2time((int)$this->phrase['from'], true), 'to' => $this->unix2time((int)$this->phrase['to'], true));
                        break;
                    default:
                        $phrase = '';
                        break;
            ...
            if ($this->search_default === false && !$this->search_columns) {
                $out .= $this->open_tag(
                    array('tag' => 'input', 'type' => 'hidden'),
                    'xcrud-columns-select ' . $this->theme_config('search_fieldlist'),
                    $attr
                );
            } else {
        ...
        switch ($attr['data-type']) {
            ...
            case 'timestamp':
                if (!isset($fieldlist['date']) or $field == $this->column) {
                    ...
                    $fieldlist['date'] .= $this->open_tag('option', '', array('value' => '')) . $this->lang('choose_range') . $this->close_tag('option');
                    ...
                    $fieldlist['date'] .= $this->single_tag(
                        'input',
                        'xcrud-datepicker-from ' . $class . ' ' . $this->theme_config('search_from'),
                        $attr
                    );
                    $attr['name'] = 'phrase][to';
                    ...
                    $fieldlist['date'] .= $this->single_tag(
                        'input',
                        'xcrud-datepicker-to ' . $class . ' ' . $this->theme_config('search_to'),
                        $attr
                    );
                }
                break;
        ...
        if ($this->search) {
            if ($this->column) {
                switch ($this->field_type[$this->column]) {
                    ...
                    case 'date':
                    case 'time':
                        $out .= $this->single_tag($tag, '', array('name' => 'phrase][from', 'value' => (isset($this->phrase['from']) ? $this->phrase['from'] : '')));
                        $out .= $this->single_tag($tag, '', array('name' => 'phrase][to', 'value' => (isset($this->phrase['to']) ? $this->phrase['to'] : '')));
                        break;
                    default:
        ...
        if ($this->result_list) {
            foreach ($this->result_list as $key => $row) {
                ...
                if (($this->is_edit || $this->is_remove || $this->is_view || $this->buttons || $this->is_duplicate || $this->grid_restrictions) && $this->task != 'print' && $this->buttons_position == 'right') {
                    $out .= $this->open_tag($item, 'xcrud-actions' . (Xcrud_config::$fixed_action_buttons ? ' xcrud-fix' : ''), $this->_cell_attrib(false, false, false, $row, false, $row_color, $row_class));
                    $out .= $this->_render_list_buttons($row);
                    $out .= $this->close_tag($item);
                }
        ...
        if ($this->sum && $this->result_list) {
            ...
            foreach ($this->columns as $field => $fitem) {
                if (isset($this->field_type[$field]) && ($this->field_type[$field] == 'password' or $this->field_type[$field] ==
                    'hidden'))
                    continue;
                $out .= $this->open_tag($item, isset($this->sum[$field]) ? $this->sum[$field]['class'] : '', $this->_cell_attrib(
                    $field,
                    isset($this->sum[$field]) ? $this->sum[$field] : null,
                    $this->order_column,
                    $this->sum,
                    true
                ));
    ...
    {
        if (
            isset($this->upload_config[$field]['width']) or isset($this->upload_config[$field]['height'])
    ...
    protected function is_edit(&$row)
    {
        if (!isset($this->grid_restrictions['edit'])) {
            ...
            return $this->_compare($row[$fname], $this->grid_restrictions['edit']['operator'], $this->replace_text_variables($this->grid_restrictions['edit']['value'], $row)) ? $this->is_edit : !$this->is_edit;
        }
    }
    protected function is_remove(&$row)
    {
        if (!isset($this->grid_restrictions['remove'])) {
            ...
            $fname = key($fdata) /*$fdata[0]['table'] . '.' . $fdata[0]['field']*/;
            return $this->_compare($row[$fname], $this->grid_restrictions['remove']['operator'], $this->replace_text_variables($this->grid_restrictions['remove']['value'], $row)) ? $this->is_remove : !$this->is_remove;
        }
    }
    protected function is_duplicate(&$row)
    {
        if (!isset($this->grid_restrictions['duplicate'])) {
            ...
            $fname = key($fdata) /*$fdata[0]['table'] . '.' . $fdata[0]['field']*/;
            return $this->_compare($row[$fname], $this->grid_restrictions['duplicate']['operator'], $this->replace_text_variables($this->grid_restrictions['duplicate']['value'], $row)) ? $this->is_duplicate : !$this->is_duplicate;
        }
    }
    protected function is_view(&$row)
    {
        if (!isset($this->grid_restrictions['view'])) {
            ...
            $fname = key($fdata) /*$fdata[0]['table'] . '.' . $fdata[0]['field']*/;
            return $this->_compare($row[$fname], $this->grid_restrictions['view']['operator'], $this->replace_text_variables($this->grid_restrictions['view']['value'], $row)) ? $this->is_view : !$this->is_view;
        }
    }
    ...
    protected function is_button($name, &$row)
    {
        if (isset($this->grid_restrictions[$name])) {
            ...
            $fname = key($fdata) /*$fdata[0]['table'] . '.' . $fdata[0]['field']*/;
            return $this->_compare($row[$fname], $this->grid_restrictions[$name]['operator'], $this->replace_text_variables($this->grid_restrictions[$name]['value'], $row)) ? true : false;
        } else {
            return true;
        }
    }
    ...
    protected function cast_number_format($number, $field, $edit = false)
    {
        ...
        $number = preg_replace('/^(.*)[\.\,' . preg_quote($this->field_attr[$field]['point'], '/') . ']+([^\.\,' . preg_quote($this->field_attr[$field]['point'], '/') . ']*)$/ui', '$1' . $loc_point . '$2', $number);
        ...
        if ($edit) {
            ...
            $out .= number_format(
                $number ? $number : 0,
                $this->field_attr[$field]['decimals'],
                $this->field_attr[$field]['point'],
